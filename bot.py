...
# =========================================================
# OKX SCALPING BOT V12.5 PA REFACTOR (FIXED) - RELAXED GATE
...
#      analysis dict / reason string, per the RELAXED definition:
#         SR + SWEEP + RECLAIM all agreeing in one direction, AND
#         (BOS OR RETEST) in that same direction => PA_COMPLETE = True.
#      (Previously required SR + SWEEP + RECLAIM + BOS + RETEST, i.e. all
#      5 stages via strict AND. This is now a 4-condition gate: 3 mandatory
#      + 1-of-2 for the final confirmation stage. This will fire more often
#      and with less confirmation per trade than the original 5/5 gate.)
...
VERSION = "V12.5-PA-FIXED-RELAXED"
...
# Break-even trigger/offset: 0.35% trigger / 0.25% offset. Sized against
# OKX's actual TOTAL round-trip fee (entry + exit combined) of ~0.12% of
# notional, confirmed directly against OKX's fee schedule (~$0.036 on a
# $30 notional) -- not the bot's original 0.05%-per-side ($0.03) estimate,
# which understated real cost. A 0.25% break-even offset locks in ~$0.075
# gross, leaving a genuine positive margin (~$0.039) above the real
# round-trip fee, instead of a paper "win" that's actually a wash or loss
# once real costs are subtracted.
...
# NOTE: max achievable score is still 9 (4 indicator votes + 5 explicit PA
# stages: SR, SWEEP, RECLAIM, BOS, RETEST) -- the stage COUNT hasn't
# changed, only how pa_complete_* is derived from those stages (see above).
# The defaults below were tuned against the old strict-AND gate; re-check
# these thresholds against the relaxed gate for your risk tolerance, since
# PA_COMPLETE will now trigger more easily.
...
# SR tolerance is now two separately configurable knobs (replaces the old
# single PA_SR_ATR_DISTANCE):
#   SR_ATR_MULT - ATR multiple used as the tolerance around a swing level.
#                 Raised again from 1.0 -> 1.3 after live DEMO testing showed
#                 SR is mandatory (see PA_COMPLETE gate below) and needs a
#                 wider tolerance to avoid spurious SR=NO vetoes -- but SR
#                 was deliberately NOT made optional, because the earlier
#                 any-4-of-5 gate (which let SR be skipped) produced larger,
#                 structurally-unanchored losing trades (e.g. -1.87%,
#                 -2.16%) versus small wins, in real DEMO results.
#   SR_PCT      - percent-of-price floor tolerance (used when ATR is very
#                 small, e.g. low-volatility conditions), so SR detection
#                 never collapses to near-zero tolerance. Value is a percent
#                 number, e.g. "0.10" means 0.10%. This was previously a
#                 hardcoded 0.0010 (0.10%) constant; it's now configurable.
# If you were previously setting PA_SR_ATR_DISTANCE in Railway, rename that
# variable to SR_ATR_MULT (same role, new name).
SR_ATR_MULT = Decimal(os.getenv("SR_ATR_MULT", "1.3"))
...
# Set to 0.06% per side so that estimated_round_trip (= notional *
# FEE_RATE_PER_SIDE * 2) equals a 0.12% TOTAL round-trip cost -- the actual
# combined entry+exit fee figure, checked directly against OKX's fee
# schedule. (Earlier this was set to 0.05% per side, understating real
# round-trip cost by roughly 3x: $0.03 assumed vs the real ~$0.036 on a $30
# notional.) This feeds both fee_buffer_ok() (the entry-time profitability
# check) and, indirectly, whether BREAK_EVEN_OFFSET_PCT above actually
# locks in more than the real trading cost when break-even triggers.
...
def _price_zone(levels, price, tol):
    """Cluster nearby swing levels into a structural S/R zone, instead of
    treating each swing as an isolated, noisy single price point.

    Previously support/resistance was just the single closest raw swing
    high/low within tolerance -- so if price sat between two nearby swing
    lows that together clearly mark the same structural zone (e.g. lows at
    100.10 and 100.05, both real reaction points), a price at 100.30 might
    be farther than tolerance from either swing INDIVIDUALLY even though it
    is clearly reacting to that same zone. Two or more swing touches within
    `tol` of each other are now merged into one zone; the zone's near edge
    (the boundary closest to `price`) is used as the representative level
    for all downstream SR-distance / sweep / reclaim comparisons.

    This directly reduces spurious SR=NO outcomes caused by using a single
    swing point, without loosening the tolerance itself -- it only merges
    swings that were already genuinely close to each other.
    """
    if not levels:
        return None
    pts = sorted(levels)
...
def price_action_structure(candles):
    ...
    RELAXED PA_COMPLETE (per direction) = SR and SWEEP/RECLAIM are both
    required (mandatory), AND at least ONE of BOS or RETEST must also be
    true for that direction. This loosens the original strict 5/5 AND-gate
    ("SR + Sweep + Reclaim + BOS + Retest -> if all same direction ->
    PA_COMPLETE = TRUE") to a 4-condition gate: 3 mandatory stages plus a
    1-of-2 final confirmation. It will trigger more often, and with a
    weaker minimum evidence bar per trade, than the strict version.

    Candle rejection and order block remain confirmations/context only;
    they do NOT feed the PA_COMPLETE gate.
    """
    c = _confirmed(candles)
    empty = {
        "sr": "NONE", "candle_rejection": "NONE", "bos": "NONE",
        "sweep": "NONE", "order_block": "NONE", "retest": "NONE",
        "sweep_reclaim": "NONE", "pa_score": 0, "pa_stage_count": 5, "pa_buy": 0, "pa_sell": 0,
        "bullish_setup": False, "bearish_setup": False,
        "pa_complete_buy": False, "pa_complete_sell": False,
        "bull_sr_ok": False, "bull_sweep_ok": False, "bull_reclaim_ok": False,
        "bull_bos_ok": False, "bull_retest_ok": False,
        "bear_sr_ok": False, "bear_sweep_ok": False, "bear_reclaim_ok": False,
        "bear_bos_ok": False, "bear_retest_ok": False,
    ...
    # Structural-zone lookup: merges nearby swing touches into one zone and
    # returns the zone edge closest to price, rather than a single raw swing.
    ...
    if dist_support <= sr_tol and dist_support <= dist_resistance:
        sr = "SUPPORT"
    elif dist_resistance <= sr_tol:
        sr = "RESISTANCE"
    else:
        sr = "NONE"
    ...
    # close reclaims it). We track and report them as two separate named
    # stages for auditability against the 5-part sequence: SR, SWEEP,
    # RECLAIM, BOS, RETEST -- even though PA_COMPLETE below now only
    # requires SR+SWEEP+RECLAIM plus one of {BOS, RETEST}.
    # DIRECTION-AUDIT FIX: bull_sr_ok / bear_sr_ok used to be derived from
    # the single `sr` mutex above (whichever of support/resistance is
    # CLOSER to current price wins, the other is forced to "NONE"). That
    # caused a real bug: once a resistance-side sequence completes
    # (SWEEP+RECLAIM+BOS+RETEST all true for the bearish/resistance side),
    # price has by definition moved AWAY from that resistance during BOS,
    # so a nearby support zone can become the new "closer" level and the
    # mutex flips `sr` to "SUPPORT" -- vetoing an otherwise fully-formed
    # bearish setup purely because support happened to be nearer to price
    # *right now*, even though the actual completed structure was at
    # resistance. SR must be checked independently per direction: is price
    # within tolerance of the support zone (for a bullish setup), and
    # independently, is price within tolerance of the resistance zone (for
    # a bearish setup) -- both can be true, or only one, without either
    ...
    # RELAXED GATE v4 (as requested):
    # Back to SR being MANDATORY (along with SWEEP and RECLAIM), with BOS
    # and RETEST as a flexible 1-of-2 final confirmation. This replaces v3
    # (any 4-of-5, no mandatory stage), which was found to let entire
    # sessions of trades through with SR never confirmed at all (since
    # SWEEP+RECLAIM+BOS+RETEST alone already summed to 4/5). SR is the
    # anchor of the whole setup -- a real support/resistance location --
    # and should never be the stage that's silently skipped.
    #   pa_complete = SR AND SWEEP AND RECLAIM AND (BOS OR RETEST)
    # This is combined with the improved SR detection above (SR_ATR_MULT,
    # SR_PCT, and structural zone clustering via _price_zone), which should
    # reduce spurious SR=NO cases without SR itself becoming optional.
    # Note pa_buy/pa_sell (the 0-5 stage counts, used for indicator scoring)
    # are UNCHANGED and still reflect exactly how many of the 5 stages are
    # true -- only the boolean gate below was changed.
    pa_complete_buy = bull_sr_ok and bull_sweep_ok and bull_reclaim_ok and (bull_bos_ok or bull_retest_ok)
    pa_complete_sell = bear_sr_ok and bear_sweep_ok and bear_reclaim_ok and (bear_bos_ok or bear_retest_ok)
    ...
    # missing mandatory stage (SR/SWEEP/RECLAIM) in the core sequence.
    ...
    return {
        ...
        # Explicit, unambiguous per-direction stage booleans -- these are the
        # ground truth used to compute pa_buy/pa_sell/pa_complete_* above, and
        # should be used for any audit/logging output instead of the merged
        # sr/sweep/bos/retest fields (which only show one side at a time and
        # default to bullish when both are None).
        "bull_sr_ok": bull_sr_ok, "bull_sweep_ok": bull_sweep_ok, "bull_reclaim_ok": bull_reclaim_ok,
        "bull_bos_ok": bull_bos_ok, "bull_retest_ok": bull_retest_ok,
        "bear_sr_ok": bear_sr_ok, "bear_sweep_ok": bear_sweep_ok, "bear_reclaim_ok": bear_reclaim_ok,
        "bear_bos_ok": bear_bos_ok, "bear_retest_ok": bear_retest_ok,
...
def analyze(symbol, oi_change=None):
    ...
    # Combined score still uses the full 0-5 PA stage count (pa_buy/pa_sell),
    # not the relaxed boolean gate -- so the score/threshold semantics are
    # unchanged; only the direction-selection gate below is relaxed.
    ...
    # RELAXED PA_COMPLETE GATE: signal direction is only chosen from a
    # direction whose PA sequence is complete under the relaxed definition
    # (pa["pa_complete_buy"] / pa["pa_complete_sell"] = SR+SWEEP+RECLAIM AND
    # (BOS OR RETEST)). If both were somehow complete simultaneously
    ...
    # Report both directions' full stage breakdown explicitly for audit,
    # independent of which single direction ends up chosen as `signal`.
    reason = (
        f"BUY[SR={'OK' if pa['bull_sr_ok'] else 'NO'} SWEEP={'OK' if pa['bull_sweep_ok'] else 'NO'} "
        f"RECLAIM={'OK' if pa['bull_reclaim_ok'] else 'NO'} BOS={'OK' if pa['bull_bos_ok'] else 'NO'} "
        f"RETEST={'OK' if pa['bull_retest_ok'] else 'NO'}]={pa['pa_buy']}/{pa['pa_stage_count']} | "
        f"SELL[SR={'OK' if pa['bear_sr_ok'] else 'NO'} SWEEP={'OK' if pa['bear_sweep_ok'] else 'NO'} "
        f"RECLAIM={'OK' if pa['bear_reclaim_ok'] else 'NO'} BOS={'OK' if pa['bear_bos_ok'] else 'NO'} "
        f"RETEST={'OK' if pa['bear_retest_ok'] else 'NO'}]={pa['pa_sell']}/{pa['pa_stage_count']} | "
        f"PA_COMPLETE_BUY={pa['pa_complete_buy']} | PA_COMPLETE_SELL={pa['pa_complete_sell']} | "
        f"CANDLE={pa['candle_rejection']} | OB={pa['order_block']} | BASE={max(base_buy,base_sell)}/4 | "
    ...
    if direction == "none":
        reason += " | PA_VETO=NEED_SR_SWEEP_RECLAIM_AND(BOS_OR_RETEST)"
    ...
    return {
        ...
        "bull_sr_ok": pa.get("bull_sr_ok"), "bull_sweep_ok": pa.get("bull_sweep_ok"),
        "bull_reclaim_ok": pa.get("bull_reclaim_ok"), "bull_bos_ok": pa.get("bull_bos_ok"),
        "bull_retest_ok": pa.get("bull_retest_ok"),
        "bear_sr_ok": pa.get("bear_sr_ok"), "bear_sweep_ok": pa.get("bear_sweep_ok"),
        "bear_reclaim_ok": pa.get("bear_reclaim_ok"), "bear_bos_ok": pa.get("bear_bos_ok"),
        "bear_retest_ok": pa.get("bear_retest_ok"),
...
def startup_checks():
    ...
    log(f"SCORE priority={max(MIN_SCORE, MAJOR_MIN_SCORE)} | non-priority={NON_PRIORITY_MIN_SCORE} | max=9 (4 indicator + 5 PA stages) | ENGINE={VERSION} | GATE=SR+SWEEP+RECLAIM+(BOS_or_RETEST) [RELAXED v4, SR mandatory]")
