...
MARGIN_USDT = Decimal(os.getenv("MARGIN_USDT", "13"))
LEVERAGE = Decimal(os.getenv("LEVERAGE", "5"))

# Fee buffer — margin سے الگ
FEE_BUFFER_USDT = Decimal(
    os.getenv("FEE_BUFFER_USDT", "0.20")
)
...
def place_order(symbol, analysis):
    ...
    with order_lock:
        ...
        log(
            ...
            f"{side.upper()} | "
            f"size={fmt(size)} | "
            f"margin={fmt(MARGIN_USDT)} | "
            f"notional={fmt(MARGIN_USDT * LEVERAGE)} | "
            f"fee_buffer={fmt(FEE_BUFFER_USDT)}"
        )
...
def startup_checks():
    ...
    log(
        f"POSITION NOTIONAL="
        f"${fmt(MARGIN_USDT * LEVERAGE)}"
    )
    log(
        f"FEE BUFFER="
        f"${fmt(FEE_BUFFER_USDT)}"
    )
    log(
        f"TOTAL BALANCE REFERENCE="
        f"${fmt(MARGIN_USDT + FEE_BUFFER_USDT)}"
    )
    log(f"SL={SL_PERCENT}%")
    log(f"TP={TP_PERCENT}%")
    log(
