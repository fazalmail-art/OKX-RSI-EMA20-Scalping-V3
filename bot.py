...
# =========================================================
# OKX SCALPING BOT V14.1 — SIX-POINT CONFLUENCE + INSTITUTIONAL RISK LAYER
#                          + PA-GATE/SCORE ALIGNMENT + TRANSPARENT DASHBOARD
# DEMO ONLY by default. No profitability guarantee. Trading involves risk
# of total capital loss. This is not financial advice.
#
# Changes vs V14.0:
#   1. V14.1 CHANGE (bug fix, not a loosening of quality): pa_score (shown
#      on the dashboard as X/5) included RETEST as its 5th stage, but the
#      actual trade-enabling gate (pa_complete_buy/sell) required
#      BOS_RECENT instead of RETEST. Since retest happens *after* BOS by
#      construction, a fully "5/5" setup routinely had a BOS that was no
#      longer "recent" once the retest printed -- so the dashboard showed
#      5/5 while the gate silently stayed closed. The gate now accepts
#      BOS_RECENT **or** RETEST_ok (retest is itself strong evidence the
#      structure is still valid), which removes the mismatch without
#      lowering the underlying evidence bar -- all 4 core stages (SR,
#      SWEEP, RECLAIM, BOS) are still mandatory.
#   2. V14.1 CHANGE: PA_BOS_MAX_AGE_BARS default 3 -> 5 (small breathing
#      room so a valid setup isn't discarded purely for timing, in
#      addition to fix #1, not instead of it).
#   3. V14.1 CHANGE: analyze() now returns an explicit ordered list of
#      "gate_stages" (SR/SWEEP/RECLAIM/BOS/RETEST with pass/fail per
#      direction) and a plain-English "block_reason" string that names the
#      exact reason a signal was NOT taken this cycle (session closed,
#      PA gate incomplete + which stage, indicator veto, score short,
#      fee buffer fail, filter fail, circuit breaker, etc). This removes
#      the need to dig through raw logs.
#   4. V14.1 CHANGE: dashboard renders a visual gate pipeline per symbol
#      (colored pass/fail badges for SR->SWEEP->RECLAIM->BOS->RETEST) and
#      a dedicated "Why no trade" column driven by block_reason.
#   5. All V14.0 institutional risk-layer features retained unchanged:
#      SQLite persistence, Telegram alerting, circuit breakers (daily
#      loss / drawdown / consecutive losses), manual kill switch, layered
#      emergency close, volatility-adjusted sizing, correlation-aware
#      exposure limits, periodic time resync, startup config validation.
#   6. All V13.1 core fixes retained unchanged (lock separation, PA-first
#      price action detection, step-based trailing, IOC fresh-price
#      slippage guard, business-vs-transient error handling, etc).
# =========================================================

VERSION = "V14.1-SIX-POINT-INSTITUTIONAL-TRANSPARENT"
...
# V14.1 CHANGE: 3 -> 5. Small, deliberate relaxation -- paired with the
# BOS_RECENT-or-RETEST_ok gate fix below, not a substitute for it.
...
# =========================================================
# PERSISTENCE LAYER (SQLite) -- unchanged from V14.0
# =========================================================
...
# =========================================================
# ALERTING LAYER (Telegram, optional) -- unchanged from V14.0
# =========================================================
...
# =========================================================
# OKX API LAYER -- unchanged from V14.0
# =========================================================
...
# =========================================================
# RISK / CIRCUIT BREAKER LAYER -- unchanged from V14.0
# =========================================================
...
# =========================================================
# CORRELATION-AWARE EXPOSURE LIMIT -- unchanged from V14.0
# =========================================================
...
# =========================================================
# VOLATILITY-ADJUSTED POSITION SIZING -- unchanged from V14.0
# =========================================================
...
# =========================================================
# LAYERED EMERGENCY CLOSE -- unchanged from V14.0
# =========================================================
...
def price_action_structure(candles):
    ...
    V14.1 CHANGE: the completion gate (pa_complete_buy/sell) now accepts
    BOS_RECENT **or** RETEST_ok, instead of requiring BOS_RECENT alone.
    Rationale: retest happens strictly after BOS in this engine's own
    detection order, so a genuinely complete 5-stage setup (SR, SWEEP,
    RECLAIM, BOS, RETEST all true -- i.e. pa_score = 5/5) could still have
    its gate closed simply because the BOS candle itself had aged past
    PA_BOS_MAX_AGE_BARS by the time the retest printed. A valid retest is
    itself confirmation that the broken structure still holds, so it is a
    legitimate substitute for "the BOS was very recent" -- not a lowering
    of the evidence bar. SR + SWEEP + RECLAIM + BOS remain unconditionally
    mandatory; only the freshness proof changed.
    """
    ...
    # V14.1 CHANGE: (bos_recent OR retest_ok) instead of bos_recent alone.
    ...
    # V14.1 CHANGE: explicit ordered gate-stage lists for the dashboard's
    # visual pipeline (SR -> SWEEP -> RECLAIM -> BOS -> RETEST).
    gate_stages_buy = [
...
def analyze(symbol, oi_change=None):
    ...
    if any(x is None for x in (e20[i], r14[i], atr_v, ml[i], ms[i], vw)):
        return {**empty, "reason": "Indicator unavailable", "block_reason": "Indicator data unavailable this cycle"}
    ...
    # V14.1 CHANGE: build a single plain-English block_reason so the
    # dashboard can show exactly why no trade happened this cycle, without
    # anyone needing to parse the raw log line.
    block_reason = None
    if signal in ("BUY", "SELL"):
        block_reason = None  # a trade signal was in fact approved
        ...
        # Tell the user exactly which stage is missing on whichever side is closer to complete.
...
def startup_checks():
    log("====================================================")
    log(f"OKX SCALPING BOT {VERSION} | PA-FIRST + INDICATOR FILTERS + RISK LAYER + TRANSPARENT GATE")
    ...
    log(f"PA_BOS_MAX_AGE_BARS={PA_BOS_MAX_AGE_BARS} | GATE=BOS_RECENT_OR_RETEST_OK (V14.1)")
    ...
    while True:
        ...
        for symbol in SYMBOLS:
            try:
                ...
                log(f"{symbol}: {analysis['signal']} {analysis.get('score',0)}/{analysis.get('max_score',9)} need={analysis.get('required_score','-')} | {analysis.get('session')} | {analysis.get('block_reason','')} | {analysis.get('reason','')}")
...
# V14.1 CHANGE: dashboard now renders a visual gate pipeline (SR->SWEEP->
# RECLAIM->BOS->RETEST as colored pass/fail badges) plus a dedicated
# "Why no trade" column driven by block_reason -- no more log-diving.
HTML = r'''<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>OKX Scalping Bot V14.1</title><style>
...
th,td{padding:7px;border-bottom:1px solid #2b313a;text-align:left;white-space:nowrap;vertical-align:middle}
...
.dot{width:22px;height:18px;border-radius:4px;font-size:9px;display:flex;align-items:center;justify-content:center;font-weight:bold;color:#0b0d10}
.dot.ok{background:#45d483}
.dot.fail{background:#3a3f47;color:#8a919c}
.why{color:#f2b84b;white-space:normal;max-width:260px}
</style></head><body>
<h2>OKX SCALP V14.1 — SIX-POINT + RISK LAYER + TRANSPARENT GATE</h2>
<div id="top"></div><div id="activity">Loading...</div>
<div class="wrap"><table><thead><tr>
<th>Pair</th><th>Signal</th><th>Score</th><th>Session</th><th>15m</th>
<th>BUY Gate (SR→SWP→RCL→BOS→RTS)</th><th>SELL Gate (SR→SWP→RCL→BOS→RTS)</th>
<th>Why no trade</th>
<th>ADX</th><th>Vol</th><th>ATR%</th><th>VolMult</th><th>Funding</th><th>OI Δ</th>
...
async function refresh(){
  try{
    ...
    document.getElementById('top').innerHTML='<div class="card">Mode: <b>'+e(s.mode)+'</b> | Auto: <b>'+e(s.auto_trade)+'</b> | Margin: <b>$'+e(s.margin)+'</b> | Lev: <b>'+e(s.leverage)+'x</b> | Exposure: <b>$'+e(s.exposure)+' / $'+e(s.max_exposure)+'</b> | Worker: <b>'+e(s.worker_error||'OK')+'</b> | Risk Halted: <b>'+e(s.trading_halted)+'</b> '+e(s.halt_reason||'')+'</div><div class="card">Public: <b>'+e(s.public_api)+'</b> | Private: <b>'+e(s.private_api)+'</b> | Snapshot age: <b>'+e(s.position_snapshot_age)+'s</b> | <button onclick="killSwitch()">KILL SWITCH</button></div>';
    ...
    for(const [sym,x] of Object.entries(s.symbols||{})){
      ...
      h+='<tr><td>'+e(sym)+'</td>'
        +'<td class="'+(sig==='BUY'?'buy':sig==='SELL'?'sell':'')+'">'+e(sig)+'</td>'
        ...
        +'<td>'+e(x.adx)+'</td><td>'+e(x.volume_ratio)+'</td><td>'+e(x.atr_pct)+'</td><td>'+e(x.vol_size_mult)+'</td>'
        +'<td>'+e(x.funding_pct)+'</td><td>'+e(x.oi_change_pct)+'</td>'
        ...
        +'<td>'+e(x.trade_status||'WAITING')+'</td>
