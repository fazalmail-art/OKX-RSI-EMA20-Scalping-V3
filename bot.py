...
FEE_BUFFER_USDT = Decimal(os.getenv("FEE_BUFFER_USDT", "0.20"))
...
def current_session():
    ...
    for start, end in SESSION_WINDOWS:
        if start <= now <= end:
            return f"{start.strftime('%H:%M')}-{end.strftime('%H:%M')} PKT"
    return "OFF_SESSION"
...
def refresh_position_mode():
    ...
    rows = data.get("data", [])
    if rows:
        position_mode = rows[0].get("posMode", "net")
    log(f"OKX POSITION MODE | {position_mode}")
    if position_mode not in ("net", "long_short_mode"):
        raise RuntimeError(f"Unsupported OKX position mode: {position_mode}")
...
def startup_checks():
    ...
    log(f"SESSIONS PKT=01:00-02:30, 06:00-07:00, 10:00-11:00, 17:40-19:00")
