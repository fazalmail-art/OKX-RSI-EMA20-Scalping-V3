...
# =========================================================
# OKX SCALPING BOT V14.3 — SIX-POINT CONFLUENCE + INSTITUTIONAL RISK LAYER
#                          + 1H TREND FILTER + SIMPLIFIED DASHBOARD
# DEMO ONLY by default. No profitability guarantee. Trading involves risk
# of total capital loss. This is not financial advice.
#
# V14.3: HTF trend filter moved from 15m to 1H (TREND_BAR default changed).
# Rationale (standard multi-timeframe practice used by discretionary and
# systematic traders alike): the trend filter should sit meaningfully
# above the entry timeframe (5m) to avoid whipsaw -- 15m is too close to
# the 5m entry frame and can flip almost as often as the entry signal
# itself, defeating the purpose of a "higher timeframe bias" filter. 1H
# gives a materially more stable directional read while still being
# responsive enough for a scalping/intraday system (as opposed to 4H/1D,
# which would be appropriate for swing trading, not scalping). TREND_BAR
# remains a configurable env var if a different HTF frame is wanted.
#
...
# available via /api/status JSON). Trend and Signal are now large colored
# badges for at-a-glance directional bias on manual/discretionary entries.
#
# V14.1: fixed PA gate / pa_score mismatch -- gate now accepts BOS_RECENT
# OR RETEST_ok (retest is downstream of BOS by construction, so a valid
# retest is itself proof the structure held). PA_BOS_MAX_AGE_BARS 3 -> 5.
# SR + SWEEP + RECLAIM + BOS remain unconditionally mandatory.
#
...
# emergency close (close-position -> manual reduce-only -> critical alert
# + halt), volatility-adjusted position sizing, correlation-aware exposure
# limits, periodic OKX time resync, startup config validation.
#
# V13.1 core (unchanged): lock separation for instrument/funding caches,
# PA-first price action detection, step-based trailing take-profit, IOC
# fresh-price slippage guard, business-vs-transient error handling.
# =========================================================

VERSION = "V14.3-SIX-POINT-INSTITUTIONAL-1H-TREND"
...
# V14.3 CHANGE: 15m -> 1H. This is the higher-timeframe bias filter used
# throughout analyze() (trend15 variable name kept for backward
# compatibility with existing state/JSON consumers, but it now reflects
# whatever TREND_BAR is configured to -- 1H by default).
...
def price_action_structure(candles):
    ...
    so a valid retest is itself acceptable proof of structural freshness,
    without weakening the 4 mandatory core stages (SR, SWEEP, RECLAIM, BOS).
    """
...
def analyze(symbol, oi_change=None):
    ...
    ml, ms = macd(values)
    vw = session_vwap(cs)
    trend15 = get_trend_from_candles(trend_cs)  # bias timeframe = TREND_BAR (default 1H)
...
def startup_checks():
    ...
    send_alert(f"{VERSION} started. Mode={'DEMO' if DEMO else 'LIVE'} AUTO_TRADE={AUTO_TRADE} TREND_BAR={TREND_BAR}", "INFO")
...
HTML = r'''<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>OKX Scalping Bot V14.3</title><style>
...
<h2>OKX SCALP V14.3 — 1H TREND FILTER + RISK LAYER</h2>
