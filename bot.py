...
# NEW: fixed take-profit, expressed as an ATR multiple from
# entry. This was completely missing before — the bot only
# ever exited on trailing stop, emergency stop, momentum
# failure, or max-hold timeout.
TP_ATR_MULT = float(os.getenv("TP_ATR_MULT", "1.20"))
...
def close_position(symbol, price, reason):
    ...
    log(f"🔴 POSITION CLOSED | {symbol} | entry={entry} | exit={price} | PnL≈{pnl:.4f} | reason={reason}")
