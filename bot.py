...
def price_action_structure(candles):
    ...
    # RELAXED GATE v3 (as requested):
    # Simple threshold on the 5 stages -- ANY 4 out of 5 (SR, SWEEP,
    # RECLAIM, BOS, RETEST) being true completes the sequence, regardless
    # of which 4. This replaces the v2 rule (SWEEP+RECLAIM mandatory + 2-of-3
    # among SR/BOS/RETEST), which effectively required a minimum of 4 anyway
    # in most cases but forced SWEEP+RECLAIM specifically. v3 has no
    # mandatory stage -- any 4-of-5 combination qualifies, including one
    # where SWEEP or RECLAIM itself is the missing stage.
    ...
    # true -- pa_complete_* is now simply pa_buy/pa_sell >= 4.
    PA_COMPLETE_MIN_STAGES = 4
    pa_complete_buy = pa_buy >= PA_COMPLETE_MIN_STAGES
    pa_complete_sell = pa_sell >= PA_COMPLETE_MIN_STAGES
...
def analyze(symbol, oi_change=None):
    ...
    if direction == "none":
        reason += " | PA_VETO=NEED_4OF5_STAGES(SR_SWEEP_RECLAIM_BOS_RETEST)"
...
def startup_checks():
    ...
    log(f"SCORE priority={max(MIN_SCORE, MAJOR_MIN_SCORE)} | non-priority={NON_PRIORITY_MIN_SCORE} | max=9 (4 indicator + 5 PA stages) | ENGINE={VERSION} | GATE=ANY_4_OF_5(SR,SWEEP,RECLAIM,BOS,RETEST) [RELAXED v3]")
