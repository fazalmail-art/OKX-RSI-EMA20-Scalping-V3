...
def price_action(cs):
    ...
    buy = [bull_sr, bull_sweep, bull_sweep, bull_bos, bull_retest]
    sell = [bear_sr, bear_sweep, bear_sweep, bear_bos, bear_retest]
    ...
    return {"sr": sr, "sweep": "BULLISH" if bull_sweep else "BEARISH" if bear_sweep else "NONE",
            "reclaim": "BULLISH" if bull_sweep else "BEARISH" if bear_sweep else "NONE",
...
def analyze(symbol, oi_change=None):
    ...
    reason = f"PA={pa['pa_buy']}/{pa['pa_sell']} | trend={tr} | ADX={fmt(adx_v,2)} | VOL={fmt(vol_ratio,2)}x | ATR={fmt(atr_pct,3)}% | score={score}/6 | blockers={','.join(blockers)}"
