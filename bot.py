...
def get_current_protection(
    ...
    """
    Exchange سے موجودہ OCO protection پڑھتا ہے۔

    یہ خاص طور پر XRP/XAU اور Railway restart
    کے بعد dashboard state recover کرنے کے لیے ہے۔
    """
...
def sync_position_state(
    ...
    """
    Open position + exchange OCO کو dashboard state
    کے ساتھ synchronize کرتا ہے۔
    """
    ...
    try:
        ...
        protection = get_current_protection(
            symbol
        )
        ...
        with state_lock:
            ...
            if protection:
                ...
                if protection.get("sl") is not None:

                    state[symbol][
                        "current_sl"
                    ] = protection["sl"]

                if protection.get("tp") is not None:
        ...
        log(
            ...
            f"{side.upper()} | "
            f"ENTRY={fmt(avg_px)} | "
            f"MARK={fmt(current_price)} | "
            f"SL={fmt(state[symbol].get('current_sl'))} | "
            f"TP={fmt(state[symbol].get('current_tp'))} | "
            f"PROTECTION="
            f"{state[symbol].get('protection')}"
        )
...
def manage_position_protection(
    ...
    # Exchange has authoritative protection
    if protection:

        if protection.get("sl") is not None:

            current_sl = protection["sl"]

        if protection.get("tp") is not None:

            current_tp = protection["tp"]
    ...
    # =====================================================
    # RECONSTRUCT ONLY IF MISSING
    # =====================================================

    if current_sl is None or current_tp is None:

        current_sl_calc, current_tp_calc = (
        ...
        if current_sl is None:
            current_sl = current_sl_calc

        if current_tp is None:
            current_tp = current_tp_calc
    ...
    if not verified:
        ...
        log(
            "CRITICAL NEW PROTECTION "
            f"NOT VERIFIED | {symbol}"
        )
...
def detect_closed_position(
    symbol
):

    """
    اگر position پہلے موجود تھی اور اب exchange پر
    position نہیں ہے تو dashboard میں CLOSED دکھاتا ہے۔
    """
...
def startup_checks():
    ...
    log(
        "DEMO + AUTO TRADE + PROTECTION RECOVERY"
    )

    log(
        "OCO + BREAK-EVEN + 0.45% TRAILING START"
    )
    ...
    # Validate all instruments independently
...
def worker():
    ...
    for symbol in SYMBOLS:
        ...
        try:
            ...
            log(
                "STARTUP POSITION RECOVERY ERROR | "
                f"{symbol} | "
                f"{error}"
            )
    ...
    while True:
        ...
        for symbol in SYMBOLS:
            ...
            try:
                ...
                detect_closed_position(
                    symbol
                )

                # -------------------------------------------------
                # Recover exchange state
                # -------------------------------------------------
