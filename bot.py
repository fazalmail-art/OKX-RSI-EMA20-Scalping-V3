...
def calculate_initial_sl_tp(side, entry, tick):
    ...
    tp = calculate_target_price(side, entry, tick, TP_PERCENT)
...
def open_partial_protection(symbol, side, entry, full_size, info, tick):
    """
    Places the full scale-out protection set for a freshly opened position:
      ...
      - one TP conditional order sized to PARTIAL_TP_FRACTION of the
        position, triggered at PARTIAL_TP_PCT profit (the "scale-out" leg)
      - one TP conditional order sized to the REMAINING fraction,
        triggered at the full TP_PERCENT target (the "runner" leg)
    ...
    """
    sl, tp = calculate_initial_sl_tp(side, entry, tick)
    ...
    partial_tp_price = calculate_target_price(side, entry, tick, PARTIAL_TP_PCT)
    last_error = None
    for attempt in range(1, 4):
        placed_ids = []
        try:
...
def place_order(symbol, analysis, snapshot):
    ...
    with order_lock:
        ...
        try:
            protection_state = open_partial_protection(symbol, side, entry, full_size, info, info["tickSz"])
        except Exception as exc:
...
def _manage_position_partial(symbol, position, avg, side, price, info, tick, profit, saved):
    ...
    if sl_algo_id is None and current_sl is None:
        ...
        if rec["sl_algo_id"] is None and rec["tp_final_algo_id"] is None and rec["tp_partial_algo_id"] is None:
            ...
            try:
                fresh = open_partial_protection(symbol, side, avg, current_size, info, tick)
                with state_lock:
                    state.setdefault(symbol, {}).update({
                        "entry_price": avg, "mark_price": price, "profit_pct": profit,
                        "protection": "ACTIVE", "position_size": dec(position.get("pos", "0")),
        ...
        current_sl = rec["current_sl"] or calculate_initial_sl_tp(side, avg, tick)[0]
        current_sl = cap_sl_distance(side, avg, current_sl, tick)
        ...
        current_tp = rec["current_tp"] or calculate_initial_sl_tp(side, avg, tick)[1]
