...
# NEW: flip every signal's direction (BUY <-> SELL) after all
# the normal confluence checks pass. Since TP/SL are still the
# same ATR distances from the (now-flipped) entry, this has the
# effect the request described: what would have been the TP
# side for the original trade becomes the SL side for the
# reversed trade, and vice versa.
# IMPORTANT: reversing because a small sample of past trades
# lost money is a hypothesis, not a guarantee — the market isn't
# symmetric, so "the opposite of a loser" isn't automatically a
# winner. ALWAYS run /api/backtest with this on vs off before
# trusting it with real orders, even on demo.
REVERSE_SIGNALS = os.getenv("REVERSE_SIGNALS", "false").lower() == "true"
...
# The "momentum failure" early-exit (close as soon as price
# crosses back over EMA20, only MIN_HOLD_SECONDS after entry)
# was firing BEFORE TP or SL ever got a chance to trigger — on
# a fast execution timeframe, price crosses EMA20 within a few
# seconds on totally normal noise, so almost every trade was
# being closed by this rule at a small loss instead of by TP/
# SL. Disabled by default so TP/SL/Trailing are what actually
# govern exits. Set to "true" to re-enable it.
ENABLE_MOMENTUM_EXIT = os.getenv("ENABLE_MOMENTUM_EXIT", "false").lower() == "true"
...
# Small grace period before the emergency SL is allowed to
# trigger. TP can still hit immediately (no reason to delay
# taking profit), but SL waiting a few seconds avoids getting
# stopped out by the initial tick/spread noise right at entry
# rather than a genuine adverse move.
EMERGENCY_SL_MIN_HOLD_SECONDS = float(os.getenv("EMERGENCY_SL_MIN_HOLD_SECONDS", "5"))
...
# Silence Flask/Werkzeug's default per-request access log
# (e.g. "GET /api/status HTTP/1.1" on every dashboard poll,
# which happens every 2 seconds). Without this, Railway's
# deploy logs fill up with HTTP noise and bury the actual bot
# events ([SIGNAL], [ORDER], [BACKTEST], etc). Real errors from
# Flask itself will still surface since only INFO-level access
# logging is suppressed, not WARNING/ERROR.
...
# Dedicated, always-visible event trails, separate from the
# noisy general log stream (which fills up fast with routine
# "candle formed" ticks across 6 symbols every 15s). These make
# it obvious when a signal fires and, separately, whether it
# actually turned into a real executed order.
signal_history = []
execution_history = []
...
def record_signal(symbol, signal):
    """Add to the dedicated signal trail (separate from the
    general log stream) every time the strategy decides a
    symbol qualifies for entry."""
...
def record_execution(symbol, side, status, details=""):
    """Add to the dedicated execution trail every time a signal
    either turns into a real placed order or fails to (status:
    'OPENED', 'CLOSED', 'FAILED', 'SIGNAL_ONLY')."""
...
def private_get(path, params=None):
    """Signed GET request. OKX requires the query string to be
    part of the signed path for GET requests."""
    query = ""
    if params:
    ...
    try:
        r = requests.get(OKX_BASE_URL + full_path, headers=headers("GET", full_path), timeout=10)
        return r.json()
    except Exception as e:
        state["last_error"] = str(e)
        log(f"[PRIVATE REST ERROR] {e}")
...
# ============================================================
# Lets you cross-check, from outside the OKX app, exactly which
# account the bot's API key is trading on (uid + demo flag).
# Compare this against the account you see in the OKX app —
# OKX's Demo Trading is a SEPARATE section of the app from your
# normal account; trades placed with x-simulated-trading:1 will
# never show up under your regular "My trades" screen, only
# under Demo Trading mode.

def verify_account():
    result = private_get("/api/v5/account/config")
    data = result.get("data", [])

    if not data:
        log(f"[ACCOUNT CHECK FAILED] {result}")
        state["account_info"] = {"error": result}
        return None
    ...
    account_info = {
        "uid": info.get("uid"),
        "acctLv": info.get("acctLv"),
        "posMode": info.get("posMode"),
        "demo_header_sent": OKX_DEMO,
    ...
    log(
        f"[ACCOUNT CHECK] uid={account_info['uid']} | "
        f"posMode={account_info['posMode']} | "
        f"demo_header_sent={OKX_DEMO} | base_url={OKX_BASE_URL} | "
        f"⚠️ Compare this UID against the account you see inside "
        f"the OKX app's Demo Trading section, not your normal account"
    )
...
def evaluate_entry(current, previous, direction, structure_atr=None):
    ...
    structure_atr, when given, is the 5-MINUTE ATR (a much more
    stable measure of real volatility) and is what TP/SL/
    trailing distances are actually sized from. The 15s ATR is
    still used to validate the execution candle itself, but
    using it to size stops made them so tight that normal price
    noise/spread was closing trades within a few seconds."""
    ...
    # Prefer the wider, more stable 5m structure ATR for sizing
    # stops. Fall back to the 15s ATR only if it's unavailable.
    sizing_atr = structure_atr if structure_atr and structure_atr > 0 else float(atr)
...
def find_entry(symbol, structure):
    ...
    return evaluate_entry(current, previous, structure["direction"], structure_atr=structure.get("atr"))
...
def market_order(symbol, side, size, reduce_only=False, retries=1):
    if not api_ready():
        return None
    ...
    while attempt <= retries:
        ...
        log(f"[ORDER REQUEST] {symbol} | {side.upper()} | contracts={size} | reduceOnly={reduce_only} | attempt={attempt}")
        ...
        # Only retry on things that look transient (network hiccup,
        # empty response). A clean rejection with a real sCode
        # (bad size, insufficient margin, etc.) won't succeed on
        # retry, so don't waste an attempt on it.
        if data:
            log("[ORDER FAILED] " + str(data[0]))
            return None

        log(f"[ORDER FAILED] No data in response (attempt {attempt}/{retries + 1})")
...
# ============================================================
# ORDER PROTECTION (exchange-side TP/SL safety net)
# ============================================================
# The bot's own manage_position() loop is what normally closes
# trades on TP/SL/Trailing/MaxHold. But that only works while
# the bot process is alive and the websocket is connected — if
# Railway restarts the service, the container crashes, or the
# connection drops, an open position would sit on OKX with NO
# protection at all.
#
# ORDER_PROTECTION places a real TP/SL algo order (OCO) on OKX
# itself the moment a position opens, so the exchange enforces
# a hard stop even if the bot goes offline. The bot still tries
# to manage exits itself first (usually faster / smarter than a
# static OCO), and cancels the algo order once it closes the
# position through its own logic.

ORDER_PROTECTION = os.getenv("ORDER_PROTECTION", "true").lower() == "true"


def place_protective_orders(symbol, side, size, tp_price, sl_price):
    if not ORDER_PROTECTION:
        return None
    ...
    payload = {
        ...
        "sz": format_decimal(size),
        "reduceOnly": True,
        "tpTriggerPx": format_decimal(tp_price),
        "tpOrdPx": "-1",   # -1 = execute as market once triggered
        "slTriggerPx": format_decimal(sl_price),
        "slOrdPx": "-1",
    }

    log(f"[PROTECTION REQUEST] {symbol} | TP={tp_price} | SL={sl_price}")

    result = private_post("/api/v5/trade/order-algo", payload)
    data = result.get("data", [])

    if not data or data[0].get("sCode") != "0":
        log(f"[PROTECTION FAILED] {symbol} | {result}")
        return None

    algo_id = data[0].get("algoId")
    log(f"[PROTECTION ACTIVE] {symbol} | algoId={algo_id} | position is now protected on-exchange")
    return algo_id


def cancel_protective_orders(symbol, algo_id):
    if not ORDER_PROTECTION or not algo_id:
        return

    payload = [{"instId": symbol, "algoId": algo_id}]
    result = private_post("/api/v5/trade/cancel-algos", payload)
    log(f"[PROTECTION CANCELLED] {symbol} | algoId={algo_id} | {result.get('data', result)}")
...
def open_position(symbol, signal):
    ...
    if size <= 0:
        ...
        record_execution(symbol, signal["side"], "FAILED", "size below OKX minimum")
        return
    ...
    if not AUTO_TRADE:
        ...
        record_execution(symbol, signal["side"], "SIGNAL_ONLY", "AUTO_TRADE is off — no order sent")
        send_whatsapp(
            "🟡 OKX SIGNAL\n"
    ...
    if not order:
        record_execution(symbol, signal["side"], "FAILED", "OKX rejected the order — see logs for sCode/msg")
        return

    atr = signal["atr"]
    if signal["side"] == "buy":
        tp_price = price + atr * TP_ATR_MULT
        sl_price = price - atr * EMERGENCY_SL_ATR
    else:
        tp_price = price - atr * TP_ATR_MULT
        sl_price = price + atr * EMERGENCY_SL_ATR

    algo_id = place_protective_orders(symbol, signal["side"], size, tp_price, sl_price)
    ...
    log(
        ...
        f"protected={'yes' if algo_id else 'no'}"
    )

    record_execution(
        symbol, signal["side"], "OPENED",
        f"entry={price} contracts={size} ordId={order.get('ordId')} protected={'yes' if algo_id else 'no'}"
    )
    ...
    send_whatsapp(
        ...
        f"Protection: {'ON (TP/SL set on exchange)' if algo_id else 'OFF — bot must stay online'}"
    )
...
def close_position(symbol, price, reason):
    ...
    # Cancel the exchange-side protective TP/SL first, so it
    # doesn't try to close the position a second time right
    # after the bot closes it here.
    if position.get("algo_id"):
        cancel_protective_orders(symbol, position["algo_id"])

    if AUTO_TRADE:
        order = market_order(symbol, close_side, position["size"], reduce_only=True)
        if not order:
            log(f"[CLOSE FAILED] {symbol}")
            record_execution(symbol, close_side, "FAILED", f"close order rejected ({reason})")
            return
    ...
    record_execution(symbol, side, "CLOSED", f"{tag} | exit={price} | PnL≈{pnl:.4f} | {reason}")
...
def manage_position(symbol):
    ...
    if side == "buy":
        ...
        if age >= EMERGENCY_SL_MIN_HOLD_SECONDS and price <= emergency_sl:
        ...
        if ENABLE_MOMENTUM_EXIT and age >= MIN_HOLD_SECONDS and price < current["ema20"]:
...
def ws_message(ws, message):
    try:
        ...
        build_15s(symbol)
        # NOTE: intentionally not logging every "candle formed"
        # tick anymore — with 6 symbols on a 15s cycle this was
        # flooding the log stream and burying signal/trade
        # events. Signals and executions now have their own
        # dedicated trails (see record_signal / record_execution).
...
def backtest_symbol(symbol, limit=1500):
    ...
    exec_df = pd.merge_asof(
        exec_df, struct_small[["ts", "direction", "atr"]].rename(columns={"atr": "structure_atr"}),
    ...
    for i in range(1, len(exec_df) - 1):
        ...
        structure_atr = current.get("structure_atr", None)

        if open_trade is None:
            signal = evaluate_entry(current, previous, direction, structure_atr=structure_atr)
            if signal:
                open_trade = {
                    "side": signal["side"],
                    "entry": signal["price"],
...
.header { padding: 18px; border-radius: 18px; background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.10); margin-bottom: 14px; }
.title { font-size: 22px; font-weight: 800; }
.badges { margin-top: 10px; display: flex; flex-wrap: wrap; gap: 6px; }
.badge { padding: 5px 11px; border-radius: 20px; background: #172554; color: #93c5fd; font-size: 12px; font-weight: 700; }
.grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr)); gap: 10px; margin-bottom: 12px; }
.card { background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.09); border-radius: 14px; padding: 14px; }
...
.exitrow { display: flex; gap: 16px; flex-wrap: wrap; font-size: 13px; color: #94a3b8; padding: 10px 4px; }
.exitrow b { font-size: 14px; }
.section { margin-top: 16px; margin-bottom: 8px; font-size: 15px; font-weight: 800; display: flex; align-items: center; justify-content: space-between; color: #cbd5e1; }
.pills { display: flex; flex-wrap: wrap; gap: 6px; }
.pill { background: rgba(255,255,255,0.06); border-radius: 20px; padding: 5px 12px; font-size: 12px; border: 1px solid rgba(255,255,255,0.08); }
.position { padding: 12px; border-radius: 12px; background: rgba(255,255,255,0.05); margin-bottom: 8px; font-size: 13px; }
.feed { max-height: 260px; overflow-y: auto; background: #020617; border-radius: 14px; padding: 12px; font-size: 12.5px; color: #cbd5e1; line-height: 1.9; }
.feed-item { border-bottom: 1px solid rgba(255,255,255,0.05); padding: 4px 0; }
details.logs summary { cursor: pointer; color: #94a3b8; font-size: 13px; padding: 6px 0; }
details.logs .feed { margin-top: 8px; font-family: monospace; font-size: 11px; }
.footer { text-align: center; color: #64748b; padding: 20px; font-size: 12px; }
button.run-bt { background: #2563eb; color: white; border: none; padding: 7px 14px; border-radius: 10px; font-weight: 700; cursor: pointer; font-size: 12px; }
button.run-bt:disabled { opacity: 0.5; cursor: default; }
table.bt { width: 100%; border-collapse: collapse; font-size: 12px; }
table.bt th, table.bt td { text-align: left; padding: 6px 8px; border-bottom: 1px solid rgba(255,255,255,0.08); }
...
<div id="rr" class="badge" style="background:#052e1a; color:#4ade80;">R:R -</div>
<div id="protectionBadge" class="badge" style="background:#052e1a; color:#4ade80;">🛡️ Protection -</div>
<div id="reverseBadge" class="badge" style="background:#3f1d1d; color:#fb7185; display:none;">⚠️ REVERSED</div>
</div>
</div>

<div class="grid">
<div class="card"><div class="label">Status</div><div id="status" class="value">-</div></div>
<div class="card"><div class="label">Connection</div><div id="ws" class="value">-</div></div>
...
<div class="card"><div class="label">Open Positions</div><div id="openCount" class="value">0</div></div>
</div>

<div class="card exitrow">
<span>🎯 TP <b id="tpHits" class="green">0</b></span>
<span>🛑 SL <b id="slHits" class="red">0</b></span>
<span>🧵 Trail <b id="trailHits" class="blue">0</b></span>
<span>⚪ Other <b id="otherExits">0</b></span>
</div>

<div class="section">📊 Markets</div>
<div id="symbols" class="pills"></div>

<div class="section">📌 Open Positions</div>
<div id="positions">No open positions</div>

<div class="section">📡 Activity</div>
<div id="activityFeed" class="feed">No activity yet</div>
...
<details class="logs">
<summary>🧾 Raw logs (tap to expand)</summary>
<div class="feed" id="logs">Loading logs...</div>
</details>
...
function renderBacktest(data) {
    ...
    document.getElementById('backtestBox').innerHTML = `
        <table class="bt">
            <thead><tr>
                <th>Symbol</th><th>Trades</th><th>Win Rate</th><th>Return</th><th>🎯</th><th>🛑</th>
            </tr></thead>
            <tbody>${rows}</tbody>
        </table>
        <div style="margin-top:8px; color:#94a3b8; font-size:11px;">
            Run at ${data.run_at} • approximated with 1m candles
        </div>
    `;
}
...
async function updateDashboard() {
    try {
        ...
        document.getElementById('openCount').innerText = Object.keys(data.positions || {}).length;
        ...
        if (data.risk_reward_ratio) {
            document.getElementById('rr').innerText = `R:R 1:${data.risk_reward_ratio}`;
        }
        document.getElementById('protectionBadge').innerText = data.order_protection ? '🛡️ Protection ON' : '🛡️ Protection OFF';
        document.getElementById('protectionBadge').style.color = data.order_protection ? '#4ade80' : '#fb7185';
        document.getElementById('reverseBadge').style.display = data.reverse_signals ? 'inline-block' : 'none';

        let symbolsHTML = (data.symbols || []).map(s => `<div class="pill">${s}</div>`).join('');
        document.getElementById('symbols').innerHTML = symbolsHTML || '<div class="pill">No active symbols</div>';
        ...
        Object.keys(positions).forEach(function(symbol) {
            ...
            positionHTML += `<div class="position">
                <b>${symbol}</b> — <span class="${p.side === 'buy' ? 'green' : 'red'}">${p.side.toUpperCase()}</span>
                | Entry: ${p.entry} | Size: ${p.size} | Margin: ${Number(p.margin || 0).toFixed(2)} USDT
            </div>`;
        });
        document.getElementById('positions').innerHTML = positionHTML || '<div class="card">No open positions</div>';

        if (data.last_backtest) {
            renderBacktest(data.last_backtest);
        }

        // Merge signals + executions into one chronological feed
        let signals = (data.signal_history || []).map(s => ({
            time: s.time,
            html: `📡 <b>${s.symbol}</b> signal — <span class="${s.side === 'buy' ? 'green' : 'red'}">${s.side.toUpperCase()}</span> @ ${s.price}`
        }));
        let executions = (data.execution_history || []).map(e => {
            let icon = { OPENED: '🟢', CLOSED: '🔵', FAILED: '🔴', SIGNAL_ONLY: '🟡' }[e.status] || '⚪';
            return {
                time: e.time,
                html: `${icon} <b>${e.symbol}</b> ${e.status} (${e.side.toUpperCase()}) — ${e.details}`
            };
        });
        let feed = signals.concat(executions).sort((a, b) => new Date(b.time) - new Date(a.time)).slice(0, 40);

        if (feed.length) {
            document.getElementById('activityFeed').innerHTML = feed.map(function(f) {
                let t = new Date(f.time).toLocaleTimeString();
                return `<div class="feed-item">[${t}] ${f.html}</div>`;
            }).join('');
        }
...
def status():
    ...
    return jsonify({
        **state,
        ...
        "signal_history": list(reversed(signal_history[-30:])),
        "execution_history": list(reversed(execution_history[-30:])),
    })
...
def api_verify_account():
    """Re-check which OKX account/UID this API key is actually
    trading on right now. Compare the returned uid against the
    account shown in the OKX app's Demo Trading section — orders
    placed with the simulated-trading header never appear under
    your normal (non-demo) account history."""
    info = verify_account()
    if not info:
        return jsonify({"error": "Could not verify account. Check API credentials / logs."}), 400
    return jsonify(info)
...
def start():
    ...
    log(f"TP/SL now sized from 5m structure ATR (not 15s execution ATR) — wider, less noise-prone")
    log(f"Emergency SL grace period: {EMERGENCY_SL_MIN_HOLD_SECONDS}s (TP can still hit instantly)")
    log(f"Risk:Reward enforced = 1 : {RISK_REWARD_RATIO}")
    log(f"Momentum-failure early exit enabled: {ENABLE_MOMENTUM_EXIT}")
    log(f"REVERSE SIGNALS: {REVERSE_SIGNALS}" + (" ⚠️ trading OPPOSITE of normal signals" if REVERSE_SIGNALS else ""))
    log(f"ORDER PROTECTION (exchange-side TP/SL): {ORDER_PROTECTION}")
