...
def open_position(symbol, signal):
    ...
    record_execution(
        ...
        f"entry={price} contracts={size} ordId={order.get('ordId')}"
    )
...
<div id="reverseBadge" class="badge" style="margin-left:8px; background:#3f1d1d; color:#fb7185; display:none;">⚠️ REVERSED MODE</div>
</div>
...
<div class="logs" id="signalLog" style="height:180px;">No signals yet</div>

<div class="section">✅ Trade Executions</div>
<div class="logs" id="executionLog" style="height:180px;">No executions yet</div>
...
async function updateDashboard() {
    try {
        ...
        let signals = data.signal_history || [];
        if (signals.length) {
            document.getElementById('signalLog').innerHTML = signals.map(function(s) {
                let sideClass = s.side === 'buy' ? 'green' : 'red';
                let t = new Date(s.time).toLocaleTimeString();
                return `[${t}] <b>${s.symbol}</b> — <span class="${sideClass}">${s.side.toUpperCase()}</span> @ ${s.price} — ${s.reason}`;
            }).join('<br>');
        }

        let executions = data.execution_history || [];
        if (executions.length) {
            document.getElementById('executionLog').innerHTML = executions.map(function(e) {
                let statusColor = { OPENED: 'green', CLOSED: 'blue', FAILED: 'red', SIGNAL_ONLY: 'yellow' }[e.status] || '';
                let t = new Date(e.time).toLocaleTimeString();
                return `[${t}] <b>${e.symbol}</b> — <span class="${statusColor}">${e.status}</span> (${e.side.toUpperCase()}) — ${e.details}`;
            }).join('<br>');
        }
