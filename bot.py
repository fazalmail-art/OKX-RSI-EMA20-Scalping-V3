...
# Signal controls. Lower than the old 5/6 gate, but still requires trend alignment.
...
def detect_signal(symbol):
    ...
    pullback_bull = any(x["low"] <= e20[-1] + av * PULLBACK_ATR_DISTANCE for x in c[-PULLBACK_LOOKBACK:]) and close > e20[-1] and bullish_candle
    pullback_bear = any(x["high"] >= e20[-1] - av * PULLBACK_ATR_DISTANCE for x in c[-PULLBACK_LOOKBACK:]) and close < e20[-1] and bearish_candle
    breakout_bull = close > recent_high and bullish_candle
    breakout_bear = close < recent_low and bearish_candle
    buy_points = [trend == "bull", close > e20[-1], close > vw, adx_v >= MIN_ADX, vol_ratio >= MIN_VOLUME_RATIO or breakout_bull, (pullback_bull or breakout_bull)]
    sell_points = [trend == "bear", close < e20[-1], close < vw, adx_v >= MIN_ADX, vol_ratio >= MIN_VOLUME_RATIO or breakout_bear, (pullback_bear or breakout_bear)]
    ...
    return {"signal": signal, "score": max(buy, sell), "buy": buy, "sell": sell, "max_score": 6, "required_score": MIN_SCORE, "trend": trend, "entry": close, "atr": av, "atr_pct": atr_pct, "adx": adx_v, "volume_ratio": vol_ratio, "ema20": e20[-1], "vwap": vw, "breakout_bull": breakout_bull, "breakout_bear": breakout_bear, "pullback_bull": pullback_bull, "pullback_bear": pullback_bear, "reason": f"trend={trend} | buy={buy}/6 sell={sell}/6 | ADX={fmt(adx_v,2)} | VOL={fmt(vol_ratio,2)}x | ATR={fmt(atr_pct,3)}% | breakout={breakout_bull}/{breakout_bear} | pullback={pullback_bull}/{pullback_bear}"}
        ...
        try:
            ...
            for symbol in SYMBOLS:
                try:
                    ...
                    print(f"[{iso_now()}] {symbol} {a.get('signal')} {a.get('score','-')}/6 | {a.get('reason')}", flush=True)
