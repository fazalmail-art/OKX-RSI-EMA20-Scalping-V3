...
# =========================================================
# OKX SCALPING BOT V15.2 — SIX-POINT CONFLUENCE + INSTITUTIONAL RISK LAYER
...
#
# V15.2: 
#   - BAR default changed 5m → 15m (late/exhausted entries kam, noise kam)
#   - CORRELATION_GROUPS now ships with a non-empty sane default
#     (majors + memes) so the correlation-exposure cap actually fires
#   - Funding-rate filter already wired in V15.1 (kept intact)
#
# V15.1: Funding-rate filter wired into the live gate.
# V15.0: Partial take-profit / scale-out system.
# =========================================================

VERSION = "V15.2-BAR15M-CORRELATION-DEFAULT"
...
# V15.2: sane non-empty default so correlation protection actually works
try:
    CORRELATION_GROUPS = json.loads(os.getenv(
        "CORRELATION_GROUPS",
        '{"majors":["BTC-USDT-SWAP","ETH-USDT-SWAP","SOL-USDT-SWAP","XRP-USDT-SWAP","NEAR-USDT-SWAP","ICP-USDT-SWAP","FIL-USDT-SWAP"],"memes":["DOGE-USDT-SWAP","SHIB-USDT-SWAP"]}'
    ))
except Exception:
    CORRELATION_GROUPS = {}
...
def analyze(symbol, oi_change=None):
    ...
    funding_pct = funding.get("current")
    funding_threshold = funding.get("threshold")
    funding_ok = True
    if funding_pct is not None and funding_threshold is not None:
        funding_ok = abs(funding_pct) < funding_threshold
    ...
    if signal in ("BUY", "SELL"):
        ...
        block_reason = f"Funding rate filter blocked entry (|{fmt(funding_pct,4)}%| >= threshold {fmt(funding_threshold,4)}%, crowded positioning)"
    elif "OI" in failed:
    ...
    funding_part = f"FUND={fmt(funding_pct,4)}%/thr={fmt(funding_threshold,4)}%" if funding_pct is not None else "FUND=warming"
    reason = (
        ...
        f"trend_frame={TREND_BAR} | session={info['name']} | priority={info['priority']} | {oi_part} | {funding_part}"
    )
    if failed:
    ...
    if sl_algo_id is None and current_sl is None:
        ...
        if rec["sl_algo_id"] is None and rec["tp_final_algo_id"] is None and rec["tp_partial_algo_id"] is None:
            ...
            try:
                ...
                log(f"PARTIAL PROTECTION REBUILD FAILED | {symbol} | {exp}")
                send_alert(f"Protection rebuild FAILED. Triggering layered emergency close. Error: {exp}", "CRITICAL", symbol)
                try:
                    emergency_close(symbol)
                except Exception as close_exc:
                    log(f"CRITICAL EMERGENCY CLOSE FAILED | {symbol} | {close_exc}")
...
def startup_checks():
    ...
    log(f"FUNDING_EXTREME_PCT_FALLBACK={FUNDING_EXTREME_PCT}% | FUNDING_LOOKBACK={FUNDING_LOOKBACK}")
    log(f"DAILY_MAX_LOSS={DAILY_MAX_LOSS_PCT}% | MAX_DRAWDOWN={MAX_DRAWDOWN_PCT}% | MAX_CONSEC_LOSSES={MAX_CONSECUTIVE_LOSSES}")
    log(f"VOL_SIZE_ADJUST={VOL_SIZE_ADJUST} | CORRELATION_GROUPS={CORRELATION_GROUPS} | MAX_CORRELATED_POSITIONS={MAX_CORRELATED_POSITIONS}")
...
def worker():
    ...
    while True:
        try:
            ...
            worker_started = False
            worker_error = str(exp)
            log(f"STARTUP RETRY | {type(exp).__name__}: {exp}")
            send_alert(f"Startup failed, retrying: {exp}", "CRITICAL")
    ...
    while True:
        ...
        for symbol in SYMBOLS:
            try:
                ...
                log(f"{symbol} ERROR | {type(exp).__name__}: {exp}\n{traceback.format_exc()}")
                with state_lock:
                    state.setdefault(symbol, {})["trade_status"] = "ERROR"
                    state[symbol]["trade_error"] = str(exp)
                db_log_event("ERROR", symbol, str(exp))

        elapsed = time.time() - cycle_start
        time.sleep(max(1, POLL_SECONDS - int(elapsed)))


HTML = r'''<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>OKX Scalping Bot V15.2</title><style>
...
<h2>OKX SCALP V15.2 — 15m ENTRY + 1H TREND + FUNDING FILTER + CORRELATION GUARD</h2>
