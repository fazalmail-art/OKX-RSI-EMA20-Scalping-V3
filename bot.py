...
# =========================================================
# OKX SCALPING BOT V12.4 PA REFACTOR
...
def price_action_structure(candles):
    ...
    Required sequence for an executable setup:
      1) meaningful support/resistance location;
      2) liquidity sweep + reclaim at that level;
      3) displacement/BOS after the reclaim;
      4) retest of the broken level or order block;
      5) final retest candle must still be recent.

    Candle rejection and order block are confirmations, not standalone signals.
    """
    ...
    if bull_reclaim_idx is not None:
        end = bull_bos_idx if bull_bos_idx is not None else len(c)
        for j in range(bull_reclaim_idx, end + 1):
            if _candle_rejection_direction(c[j]) == "BULLISH":
                bull_candle_idx = j
    if bear_reclaim_idx is not None:
        end = bear_bos_idx if bear_bos_idx is not None else len(c)
        for j in range(bear_reclaim_idx, end + 1):
    ...
    # PA score = the four actual sequential stages of the setup.
    # This fixes the previous weighting bug where a complete sequence could still
    # show PA=2/4 because candle rejection and OB were incorrectly embedded in
    # the core stages. Candle rejection and OB are now separate confirmations.
    ...
    bull_stage = [
        sr == "SUPPORT",
        bull_sweep_idx is not None and bull_reclaim_idx is not None,
        bull_bos_idx is not None and bull_bos_idx > bull_reclaim_idx,
        bull_retest_recent and bull_retest_idx is not None and bull_retest_idx > bull_bos_idx,
    ]
    bear_stage = [
        sr == "RESISTANCE",
        bear_sweep_idx is not None and bear_reclaim_idx is not None,
        bear_bos_idx is not None and bear_bos_idx > bear_reclaim_idx,
        bear_retest_recent and bear_retest_idx is not None and bear_retest_idx > bear_bos_idx,
    ]
    pa_buy = sum(bull_stage)
    pa_sell = sum(bear_stage)

    bull_complete = all(bull_stage)
    bear_complete = all(bear_stage)
    ...
    # The entry gate is explicitly PA-first: no trade without a complete
    # SR -> Sweep/Reclaim -> BOS -> Recent Retest sequence. Candle rejection
    # and OB improve context but are not used to distort the 4-stage PA score.
    bullish_setup = (
        bull_complete
    )
    bearish_setup = (
    ...
    return {
        ...
        "pa_buy": pa_buy,
        "pa_sell": pa_sell,
        "pa_sequence_buy": "COMPLETE" if bull_complete else "INCOMPLETE",
        "pa_sequence_sell": "COMPLETE" if bear_complete else "INCOMPLETE",
...
def analyze(symbol, oi_change=None):
    ...
    empty = {
        "signal": "NONE", "score": 0, "max_score": 8, "required_score": 99,
    ...
    values = [x["close"] for x in cs]
    i = len(values) - 1
    e20 = ema(values, 20); r14 = rsi(values, 14); atr_v = atr(cs); adx_v = adx(cs); ml, ms = macd(values)
    vw = session_vwap(cs); trend15 = get_trend_from_candles(trend_cs); funding = get_funding_snapshot(symbol)
    ...
    # Max score remains 8: 4 indicator votes + 4 PA stages. PA is NOT double-counted.
    ...
    direction = "buy" if pa["bullish_setup"] and pa["pa_buy"] >= pa["pa_sell"] else \
                "sell" if pa["bearish_setup"] and pa["pa_sell"] > pa["pa_buy"] else "none"
    signal = "NONE"
    if direction == "buy" and buy >= required and pa["bullish_setup"]:
        signal = "BUY"
    elif direction == "sell" and sell >= required and pa["bearish_setup"]:
        signal = "SELL"

    # Indicator vetoes are retained, but they cannot create a trade without PA.
    ...
    reason = (
        ...
        f"RETEST={pa['retest']} | PA={pa['pa_score']}/4 | BASE={max(base_buy,base_sell)}/4 | "
        f"ADX={fmt(adx_v,2)} | VOL={fmt(vol_ratio,2)}x | ATR={fmt(atr_pct,3)}% | "
        f"session={info['name']} | priority={info['priority']} | "
        f"OI={fmt(oi_change,3)}%" if oi_change is not None else
        f"SR={pa['sr']} | CANDLE={pa['candle_rejection']} | BOS={pa['bos']} | "
        f"SWEEP={pa['sweep']} | RECLAIM={pa['sweep_reclaim']} | OB={pa['order_block']} | "
        f"RETEST={pa['retest']} | PA={pa['pa_score']}/4 | BASE={max(base_buy,base_sell)}/4 | "
        f"ADX={fmt(adx_v,2)} | VOL={fmt(vol_ratio,2)}x | ATR={fmt(atr_pct,3)}% | "
        f"session={info['name']} | priority={info['priority']} | OI=warming"
    )
    if failed:
    ...
    if direction == "none":
        reason += " | PA_VETO=NO_COMPLETE_SR_SWEEP_RECLAIM_BOS_RETEST"
    elif signal == "NONE" and direction == "buy" and not pa["bullish_setup"]:
        reason += " | PA_VETO=BUY_SEQUENCE_INCOMPLETE"
    elif signal == "NONE" and direction == "sell" and not pa["bearish_setup"]:
        reason += " | PA_VETO=SELL_SEQUENCE_INCOMPLETE"

    return {
        "signal": signal, "score": score, "buy": buy, "sell": sell, "max_score": 8,
        ...
        "pa_sequence_sell": pa.get("pa_sequence_sell"), "candle_confirm_buy": pa.get("candle_confirm_buy"),
...
def startup_checks():
    ...
    log(f"SCORE priority={max(MIN_SCORE, MAJOR_MIN_SCORE)} | non-priority={NON_PRIORITY_MIN_SCORE} | max=8 | ENGINE=V12.4-PA | SR+SWEEP/RECLAIM+BOS+CANDLE+OB+RECENT-RETEST")
    ...
    while True:
        ...
        for symbol in SYMBOLS:
            try:
                ...
                log(f"{symbol}: {analysis['signal']} {analysis.get('score',0)}/{analysis.get('max_score',8)} need={analysis.get('required_score','-')} | {analysis.get('session')} | {analysis.get('reason','')}")
...
HTML = r'''<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>OKX Scalping Bot V12.4</title><style>body{font-family:Arial,sans-serif;background:#0b0d10;color:#eee;margin:0;padding:14px}.card{background:#15191f;border:1px solid #2b313a;border-radius:10px;padding:10px;margin-bottom:8px}.wrap{overflow:auto}table{width:100%;border-collapse:collapse;font-size:12px;min-width:1250px}th,td{padding:7px;border-bottom:1px solid #2b313a;text-align:left;white-space:nowrap}th{background:#171b21;position:sticky;top:0}.buy{color:#45d483;font-weight:bold}.sell{color:#ff6565;font-weight:bold}.active{color:#45d483;font-weight:bold}.danger{color:#ff6565;font-weight:bold}</style></head><body><h2>OKX SCALP V12.4 — PA-First Sequential Price Action</h2><div id="top"></div><div id="activity">Loading...</div><div class="wrap"><table><thead><tr><th>Pair</th><th>Signal</th><th>Score</th><th>Priority</th><th>Session</th><th>15m</th><th>SR</th><th>Candle</th><th>BOS</th><th>Sweep</th><th>Reclaim</th><th>OB</th><th>Retest</th><th>PA</th><th>ADX</th><th>Vol</th><th>ATR%</th><th>Funding</th><th>OI Δ</th><th>Entry</th><th>Mark</th><th>P/L%</th><th>SL</th><th>TP</th><th>Protection</th><th>Status</th></tr></thead><tbody id="rows"></tbody></table></div><script>function e(x){return String(x??"-").replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]))}async function refresh(){try{const s=await fetch('/api/status').then(r=>r.json());document.getElementById('top').innerHTML='<div class="card">Mode: <b>'+e(s.mode)+'</b> | Auto: <b>'+e(s.auto_trade)+'</b> | Margin: <b>$'+e(s.margin)+'</b> | Lev: <b>'+e(s.leverage)+'x</b> | Exposure: <b>$'+e(s.exposure)+' / $'+e(s.max_exposure)+'</b> | Worker: <b>'+e(s.worker_error||'OK')+'</b> | PA Gate: <b>SWEEP → BOS → RETEST</b></div><div class="card">Public: <b>'+e(s.public_api)+'</b> | Private: <b>'+e(s.private_api)+'</b> | Position snapshot: <b>'+e(s.position_snapshot_age)+'s</b></div>';document.getElementById('activity').textContent='Last activity: '+e(s.last_activity)+' | '+e(s.updated);let h='';for(const [sym,x] of Object.entries(s.symbols||{})){const sig=x.signal||'NONE';h+='<tr><td>'+e(sym)+'</td><td class="'+(sig==='BUY'?'buy':sig==='SELL'?'sell':'')+'">'+e(sig)+'</td><td>'+e(x.score)+'/'+e(x.max_score)+' need '+e(x.required_score)+'</td><td>'+e(x.priority_session)+'</td><td>'+e(x.session)+'</td><td>'+e(x.trend15)+'</td><td>'+e(x.sr)+'</td><td>'+e(x.candle_rejection)+'</td><td>'+e(x.bos)+'</td><td>'+e(x.sweep)+'</td><td>'+e(x.sweep_reclaim)+'</td><td>'+e(x.order_block)+'</td><td>'+e(x.retest)+'</td><td>'+e(x.pa_score)+'/4</td><td>'+e(x.adx)+'</td><td>'+e(x.volume_ratio)+'</td><td>'+e(x.atr_pct)+'</td><td>'+e(x.funding_pct)+'</td><td>'+e(x.oi_change_pct)+'</td><td>'+e(x.entry_price||x.entry)+'</td><td>'+e(x.mark_price)+'</td><td>'+e(x.profit_pct)+'</td><td>'+e(x.current_sl)+'</td><td>'+e(x.current_tp)+'</td><td class="'+(x.protection==='ACTIVE'?'active':'danger')+'">'+e(x.protection||'NONE')+'</td><td>'+e(x.trade_status||'WAITING')+'</td></tr>'}document.getElementById('rows').innerHTML=h}catch(err){document.getElementById('activity').textContent='Dashboard error: '+err}}refresh();setInterval(refresh,5000)</script></body></html>'''
