...
NON_PRIORITY_MIN_SCORE = int(os.getenv("NON_PRIORITY_MIN_SCORE", "6"))  # required score OFF-session (now allowed to trade, tightest gate)
...
def _manage_position_oco_legacy(symbol, position, avg, side, price, info, tick, profit, saved):
    """V14.3 single-stage OCO management path, used only when
    PARTIAL_TP_ENABLE=false. Unchanged from earlier versions."""
        ...
        if protection_active:
            ...
            try:
                if side == "buy" and current_tp > avg:
                    ...
                    ratio = Decimal("0")
                step_level = max(0, int(ratio.to_integral_value(rounding=ROUND_DOWN)) - 1)
    ...
    with state_lock:
        state.setdefault(symbol, {}).update({
            ...
            "step_level": step_level, "using_partial": False,
        })
