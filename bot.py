...
# =========================================================
# OKX SCALPING BOT V14.0 — SIX-POINT CONFLUENCE + INSTITUTIONAL RISK LAYER
# Price-action-first sequential confirmation engine with indicator filters,
# plus persistence, alerting, circuit breakers, layered emergency close,
# volatility-adjusted sizing, and correlation-aware exposure limits.
# DEMO ONLY by default. No profitability guarantee. Trading involves risk
# of total capital loss. This is not financial advice.
#
# Changes vs V13.1:
#   1. SQLite persistence for trades, state snapshots, and events/alerts.
#      Previously everything lived in RAM only and was lost on restart.
#   2. Telegram alerting for critical events (protection failed, emergency
#      close, circuit breaker triggered, order errors). Optional — bot runs
#      fine without credentials, alerts are just skipped with a log line.
#   3. Circuit breakers: daily max loss, consecutive loss halt, max
#      drawdown halt. All three block NEW entries but let existing
#      positions continue to be managed/protected.
#   4. Kill switch: a manual HTTP endpoint (/api/kill) and a state flag that
#      immediately blocks new entries. Existing positions still protected.
#   5. Layered emergency close: close-position endpoint first, then a
#      manual reduce-only market order fallback, then a Telegram alert if
#      both fail, then the bot marks the symbol HALTED so it is not
#      re-entered blindly.
#   6. Periodic OKX time resync during the run (not just at startup) to
#      avoid signature timestamp drift over long uptimes.
#   7. Startup config validation with sane range checks; refuses to start
#      auto-trading with clearly broken configuration.
#   8. Volatility-adjusted position sizing: base margin is scaled down when
#      ATR% is elevated vs a rolling baseline, and up (capped) when it is
#      low, so risk-per-trade stays roughly constant across symbols.
#   9. Correlation-aware exposure limit: symbols can be grouped (e.g.
#      "majors", "alts") and the bot caps concurrent open positions per
#      group, so diversification is real rather than illusory.
#  10. All prior V13.1 fixes retained unchanged (lock separation, PA-first
#      gate, step-based trailing, IOC fresh-price slippage guard, etc).
# =========================================================

VERSION = "V14.0-SIX-POINT-INSTITUTIONAL"
...
# ---- NEW: institutional risk-layer config ----
...
try:
    ...
    CORRELATION_GROUPS = {}
KILL_SWITCH_TOKEN = os.getenv("KILL_SWITCH_TOKEN", "")  # simple shared-secret for /api/kill
...
# ---- NEW: runtime risk-state (in-memory, mirrored to DB) ----
risk_state = {
...
# =========================================================
# NEW: ALERTING LAYER (Telegram, optional)
# =========================================================

def send_alert(message, level="INFO", symbol=None):
    """
    Best-effort alert. Never raises -- an alerting failure must not crash
    or block the trading loop. Always logged locally and to DB regardless
    of whether Telegram is configured.
    """
...
# =========================================================
# OKX API LAYER (unchanged core, + periodic resync)
# =========================================================
...
def maybe_resync_time():
    # NEW: previously time was only synced once at startup, letting clock
    # drift build up over long-running deployments and eventually breaking
    # request signatures. Resync periodically from the worker loop.
...
def get_account_equity():
    # NEW: used by circuit breakers (daily loss / drawdown). Falls back to
    # None on failure so callers can decide to skip the check rather than
    # crash the loop.
    try:
...
# =========================================================
# NEW: RISK / CIRCUIT BREAKER LAYER
# =========================================================
...
def risk_roll_day_if_needed():
    ...
    with risk_lock:
        if risk_state.get("day") != today:
            ...
            risk_state["consecutive_losses"] = 0
            # Daily halt resets each day; drawdown halt (below) does not.
...
def risk_check_circuit_breakers():
    """
    Returns (ok_to_trade: bool, reason: str). Called before opening any new
    position. Never blocks management of existing positions.
    """
    ...
    equity = get_account_equity()
    if equity is None:
        return True, ""  # fail-open on data unavailability; existing position caps still apply
...
# =========================================================
# NEW: CORRELATION-AWARE EXPOSURE LIMIT
# =========================================================
...
def symbol_correlation_group(symbol):
    for group_name, members in CORRELATION_GROUPS.items():
        if symbol in members:
            return group_name
    return None  # ungrouped symbols are not limited by this check
...
def volatility_size_multiplier(atr_pct):
    """
    Scales the base margin so that risk-per-trade is roughly constant
    across symbols of very different volatility. High ATR% -> smaller size.
    Low ATR% -> slightly larger size, capped. Disabled via VOL_SIZE_ADJUST=false.
    """
...
# =========================================================
# NEW: LAYERED EMERGENCY CLOSE
# =========================================================

def _manual_reduce_only_close(symbol, position):
    """Fallback layer 2: direct reduce-only market order instead of the
    close-position convenience endpoint, in case that endpoint itself is
    failing (different code path on OKX's side)."""
...
def emergency_close(symbol, max_attempts=3):
    ...
    Layered emergency close:
      1) close-position endpoint (fast path), retried with backoff
      2) manual reduce-only market order (different code path) as fallback
      3) if both fail: alert immediately and mark the symbol HALTED so the
         bot does not blindly re-enter a symbol with a possibly-unprotected
         position still open.
    Never silently swallows a total failure.
    """
    last_exc = None
    ...
    # Layer 2: manual reduce-only market order
    for attempt in range(1, max_attempts + 1):
        try:
    ...
    # Both layers failed -- this is the scenario that must never be silent.
    with state_lock:
...
def price_action_structure(candles):
    """Unchanged from V13.1 -- price-action-first sequence using closed
    candles only (SR -> SWEEP -> RECLAIM -> BOS, RETEST as separate score
    point). See original docstring for full rationale; logic untouched."""
    ...
    pa_complete_buy = bull_sr_ok and bull_sweep_ok and bull_reclaim_ok and bull_bos_ok and bull_bos_recent
    pa_complete_sell = bear_sr_ok and bear_sweep_ok and bear_reclaim_ok and bear_bos_ok and bear_bos_recent
...
def analyze(symbol, oi_change=None):
    ...
    if direction == "none":
        reason += " | PA_VETO=CORE_PA_NEEDS_SR_SWEEP_RECLAIM_AND_FRESH_BOS"
...
def place_order(symbol, analysis, snapshot):
    ...
    # NEW: circuit breakers checked first -- cheapest and most important gate.
    ...
    with order_lock:
        ...
        order_state = None
        for _ in range(8):  # widened slightly from 6 -> 8 to reduce false NOT_FILLED
            time.sleep(1)
            try:
        ...
        # NEW: protection genuinely failed after 3 tries -- go through the
        # layered emergency close instead of a single best-effort call.
        ...
        try:
            ...
            # Both OCO placement and emergency close failed -- worst case.
            # Already alerted with CRITICAL inside emergency_close(). Halt
            # the symbol and keep the bot alive for everything else.
            with state_lock:
                state.setdefault(symbol, {})["halted"] = True
...
def detect_and_record_closed_positions(prev_snapshot, new_snapshot):
    """
    NEW: when a symbol disappears from the position snapshot between
    cycles, the position closed (TP/SL hit or manual close). Record the
    outcome into the DB and feed it into the consecutive-loss circuit
    breaker -- previously closures were never logged and never fed back
    into risk tracking.
    """
...
def validate_config():
    """
    NEW: startup sanity checks. Refuses to enable AUTO_TRADE with obviously
    broken configuration instead of silently running with nonsense values.
    """
    problems = []
    if MARGIN_USDT <= 0:
...
def startup_checks():
    log("====================================================")
    log(f"OKX SCALPING BOT {VERSION} | PA-FIRST + INDICATOR FILTERS + RISK LAYER")
...
def worker():
    ...
    while True:
        ...
        maybe_resync_time()  # NEW: periodic drift correction
        ...
        # NEW: detect closures for PnL logging + consecutive-loss tracking
        try:
...
HTML = r'''<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>OKX Scalping Bot V14.0</title><style>body{font-family:Arial,sans-serif;background:#0b0d10;color:#eee;margin:0;padding:14px}.card{background:#15191f;border:1px solid #2b313a;border-radius:10px;padding:10px;margin-bottom:8px}.wrap{overflow:auto}table{width:100%;border-collapse:collapse;font-size:12px;min-width:1400px}th,td{padding:7px;border-bottom:1px solid #2b313a;text-align:left;white-space:nowrap}th{background:#171b21;position:sticky;top:0}.buy{color:#45d483;font-weight:bold}.sell{color:#ff6565;font-weight:bold}.active{color:#45d483;font-weight:bold}.danger{color:#ff6565;font-weight:bold}button{background:#ff6565;color:#fff;border:none;padding:6px 12px;border-radius:6px;cursor:pointer}</style></head><body><h2>OKX SCALP V14.0 — SIX-POINT + RISK LAYER</h2><div id="top"></div><div id="activity">Loading...</div><div class="wrap"><table><thead><tr><th>Pair</th><th>Signal</th><th>Score</th><th>Priority</th><th>Session</th><th>15m</th><th>SR</th><th>Candle</th><th>BOS</th><th>Sweep</th><th>Reclaim</th><th>OB</th><th>Retest</th><th>PA</th><th>PA_COMPLETE(B/S)</th><th>ADX</th><th>Vol</th><th>ATR%</th><th>VolMult</th><th>Funding</th><th>OI Δ</th><th>Entry</th><th>Mark</th><th>P/L%</th><th>SL</th><th>TP</th><th>Step</th><th>Protection</th><th>Status</th></tr></thead><tbody id="rows"></tbody></table></div><script>function e(x){return String(x??"-").replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]))}async function refresh(){try{const s=await fetch('/api/status').then(r=>r.json());document.getElementById('top').innerHTML='<div class="card">Mode: <b>'+e(s.mode)+'</b> | Auto: <b>'+e(s.auto_trade)+'</b> | Margin: <b>$'+e(s.margin)+'</b> | Lev: <b>'+e(s.leverage)+'x</b> | Exposure: <b>$'+e(s.exposure)+' / $'+e(s.max_exposure)+'</b> | Worker: <b>'+e(s.worker_error||'OK')+'</b> | Risk Halted: <b>'+e(s.trading_halted)+'</b> '+e(s.halt_reason||'')+'</div><div class="card">Public: <b>'+e(s.public_api)+'</b> | Private: <b>'+e(s.private_api)+'</b> | Snapshot age: <b>'+e(s.position_snapshot_age)+'s</b> | <button onclick="killSwitch()">KILL SWITCH</button></div>';document.getElementById('activity').textContent='Last activity: '+e(s.last_activity)+' | '+e(s.updated);let h='';for(const [sym,x] of Object.entries(s.symbols||{})){const sig=x.signal||'NONE';h+='<tr><td>'+e(sym)+'</td><td class="'+(sig==='BUY'?'buy':sig==='SELL'?'sell':'')+'">'+e(sig)+'</td><td>'+e(x.score)+'/'+e(x.max_score)+' need '+e(x.required_score)+'</td><td>'+e(x.priority_session)+'</td><td>'+e(x.session)+'</td><td>'+e(x.trend15)+'</td><td>'+e(x.sr)+'</td><td>'+e(x.candle_rejection)+'</td><td>'+e(x.bos)+'</td><td>'+e(x.sweep)+'</td><td>'+e(x.sweep_reclaim)+'</td><td>'+e(x.order_block)+'</td><td>'+e(x.retest)+'</td><td>'+e(x.pa_score)+'/5</td><td>'+e(x.pa_complete_buy)+'/'+e(x.pa_complete_sell)+'</td><td>'+e(x.adx)+'</td><td>'+e(x.volume_ratio)+'</td><td>'+e(x.atr_pct)+'</td><td>'+e(x.vol_size_mult)+'</td><td>'+e(x.funding_pct)+'</td><td>'+e(x.oi_change_pct)+'</td><td>'+e(x.entry_price||x.entry)+'</td><td>'+e(x.mark_price)+'</td><td>'+e(x.profit_pct)+'</td><td>'+e(x.current_sl)+'</td><td>'+e(x.current_tp)+'</td><td>'+e(x.step_level)+'</td><td class="'+(x.protection==='ACTIVE'?'active':'danger')+'">'+e(x.protection||'NONE')+'</td><td>'+e(x.trade_status||'WAITING')+'</td></tr>'}document.getElementById('rows').innerHTML=h}catch(err){document.getElementById('activity').textContent='Dashboard error: '+err}}function killSwitch(){const token=prompt('Enter kill-switch token:');if(token===null)return;fetch('/api/kill',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({token})}).then(r=>r.json()).then(d=>alert(JSON.stringify(d)))}refresh();setInterval(refresh,5000)</script></body></html>'''
...
@app.post("/api/kill")
def api_kill():
    # NEW: manual kill switch. Requires KILL_SWITCH_TOKEN to be set in env
    # and matched here, so it cannot be triggered by a random request.
