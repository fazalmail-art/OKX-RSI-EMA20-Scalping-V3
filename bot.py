...
# =========================================================
# OKX SCALPING BOT V12.3 PA REFACTOR
...
def price_action_structure(candles):
    ...
    # PA score: four non-duplicated stages. Retest is mandatory for a trade setup,
    # but OB/rejection are additional confirmation rather than artificial points.
    ...
    bull_stage = [
        ...
        bull_bos_idx is not None and bull_candle_idx is not None,
        bull_retest_recent and (ob == "BULLISH"),
    ]
    bear_stage = [
        sr == "RESISTANCE",
        bear_reclaim_idx is not None,
        bear_bos_idx is not None and bear_candle_idx is not None,
        bear_retest_recent and (ob == "BEARISH"),
    ]
    pa_buy = sum(bull_stage)
    pa_sell = sum(bear_stage)

    # The entry gate is explicitly PA-first: no trade without sweep + BOS + recent retest.
    bullish_setup = (
        sr == "SUPPORT" and bull_sweep_idx is not None and bull_bos_idx is not None and
        bull_retest_recent and bull_retest_idx is not None
    )
    bearish_setup = (
        sr == "RESISTANCE" and bear_sweep_idx is not None and bear_bos_idx is not None and
        bear_retest_recent and bear_retest_idx is not None
    )
...
def analyze(symbol, oi_change=None):
    ...
    if direction == "none":
        reason += " | PA_VETO=NO_COMPLETE_SWEEP_BOS_RETEST"
    ...
    return {
        ...
        "pa_sell": pa["pa_sell"], "base_score": max(base_buy, base_sell),
...
def startup_checks():
    ...
    log(f"SCORE priority={max(MIN_SCORE, MAJOR_MIN_SCORE)} | non-priority={NON_PRIORITY_MIN_SCORE} | max=8 | ENGINE=V12.3-PA | SR+SWEEP/RECLAIM+BOS+CANDLE+OB+RECENT-RETEST")
