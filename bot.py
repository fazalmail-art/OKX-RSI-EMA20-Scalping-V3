...
PA_SR_ATR_DISTANCE = Decimal(os.getenv("PA_SR_ATR_DISTANCE", "0.75"))
...
def price_action_structure(candles):
    ...
    sr_tol = max(av * PA_SR_ATR_DISTANCE, price * Decimal("0.0010"))
    ...
    support = max(support_candidates) if support_candidates else min(x["low"] for x in c[max(0, level_end-12):level_end])
    resistance = min(resistance_candidates) if resistance_candidates else max(x["high"] for x in c[max(0, level_end-12):level_end])
    ...
    # RELAXED GATE v2 (as requested):
    # SWEEP and RECLAIM remain mandatory for both directions -- this is the
    # core event (liquidity grab + reclaim of the level) and is never
    # optional. SR, BOS, and RETEST are now a flexible group: at least 2
    # of those 3 must be true. This is looser than v1 (which required SR
    # always + one of BOS/RETEST) because SR itself can now be missing as
    # long as BOS and RETEST both confirm; conversely BOS+RETEST both being
    # absent still fails the gate even if SR alone is present.
    # Examples:
    #   SR=NO,  BOS=OK, RETEST=OK -> 2/3 -> PASSES (was blocked in v1)
    #   SR=OK,  BOS=NO, RETEST=NO -> 1/3 -> still FAILS (correctly weak)
    #   SR=OK,  BOS=OK, RETEST=NO -> 2/3 -> PASSES
    ...
    # true -- only the boolean gate below was relaxed.
    bull_flex_count = sum([bull_sr_ok, bull_bos_ok, bull_retest_ok])
    bear_flex_count = sum([bear_sr_ok, bear_bos_ok, bear_retest_ok])
    pa_complete_buy = bull_sweep_ok and bull_reclaim_ok and bull_flex_count >= 2
    pa_complete_sell = bear_sweep_ok and bear_reclaim_ok and bear_flex_count >= 2
...
def analyze(symbol, oi_change=None):
    ...
    if direction == "none":
        reason += " | PA_VETO=NO_COMPLETE_SWEEP_RECLAIM_AND_2OF3(SR_BOS_RETEST)"
...
def startup_checks():
    ...
    log(f"SCORE priority={max(MIN_SCORE, MAJOR_MIN_SCORE)} | non-priority={NON_PRIORITY_MIN_SCORE} | max=9 (4 indicator + 5 PA stages) | ENGINE={VERSION} | GATE=SWEEP+RECLAIM+2of3(SR,BOS,RETEST) [RELAXED v2]")
