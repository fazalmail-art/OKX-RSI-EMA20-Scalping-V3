...
# =========================================================
# OKX SCALPING BOT V12.5 PA REFACTOR (FIXED)
...
VERSION = "V14.0-5M-BOS-RETEST-15S-CONFIRM-FIXED"
...
ENTRY_BAR_15S = os.getenv("ENTRY_BAR_15S", "1s")
ENTRY_CONFIRM_SECONDS = int(os.getenv("ENTRY_CONFIRM_SECONDS", "15"))
RETEST_REQUIRED = os.getenv("RETEST_REQUIRED", "true").lower() == "true"
...
SYMBOLS = [x.strip() for x in os.getenv(
    "SYMBOLS",
    "BTC-USDT-SWAP,ETH-USDT-SWAP,SOL-USDT-SWAP,XRP-USDT-SWAP,DOGE-USDT-SWAP,HYPE-USDT-SWAP"
).split(",") if x.strip()]
...
def price_action_structure(candles):
    ...
    # EXPLICIT GATE (as requested):
    # PA_COMPLETE[direction] = SR AND SWEEP AND RECLAIM AND BOS AND RETEST,
    # all true and all agreeing on the same direction. `all(bull_stage)` is
...
def get_15s_confirmation(symbol):
    """Build completed 15-second candles from OKX 1-second history candles.
    REST is used intentionally: no websocket dependency, so the old candle1s
    websocket 60018/404 issue cannot stop the bot. OKX documents 1s candles
    for SWAP products on the REST history-candles endpoint.
    """
    rows = public_get("/api/v5/market/history-candles", {
        "instId": symbol, "bar": ENTRY_BAR_15S, "limit": "100"
    }).get("data", [])
    raw = []
    ...
    for r in reversed(rows):
        if len(r) < 9 or r[8] != "1":
            continue
        ts = int(r[0])
        # Never use a candle that can still be forming.
        if ts + 1000 > now_ms:
            continue
        raw.append({"ts": ts, "open": dec(r[1]), "high": dec(r[2]),
                    "low": dec(r[3]), "close": dec(r[4]), "volume": dec(r[5])})
    if len(raw) < ENTRY_CONFIRM_SECONDS:
        return {"ok": False, "signal": "NONE", "reason": "15S data warming"}

    buckets = {}
    for x in raw:
        bucket = (x["ts"] // (ENTRY_CONFIRM_SECONDS * 1000)) * (ENTRY_CONFIRM_SECONDS * 1000)
        buckets.setdefault(bucket, []).append(x)
    bars = []
    for ts in sorted(buckets):
        q = buckets[ts]
        if len(q) < ENTRY_CONFIRM_SECONDS:
            continue
        bars.append({"ts": ts, "open": q[0]["open"], "high": max(x["high"] for x in q),
                     "low": min(x["low"] for x in q), "close": q[-1]["close"],
                     "volume": sum(x["volume"] for x in q)})
    if not bars:
        return {"ok": False, "signal": "NONE", "reason": "No complete 15S candle"}
    b = bars[-1]
    rng = max(b["high"] - b["low"], Decimal("0.00000001"))
    body = abs(b["close"] - b["open"])
    body_ratio = body / rng
    bullish = b["close"] > b["open"]
    bearish = b["close"] < b["open"]
    close_high = (b["close"] - b["low"]) / rng
    close_low = (b["high"] - b["close"]) / rng
    signal = "NONE"
    if bullish and body_ratio >= Decimal("0.25") and close_high >= Decimal("0.55"):
        signal = "BUY"
    elif bearish and body_ratio >= Decimal("0.25") and close_low >= Decimal("0.55"):
        signal = "SELL"
    return {"ok": True, "signal": signal, "ts": b["ts"], "open": b["open"],
            "high": b["high"], "low": b["low"], "close": b["close"],
            "body_ratio": body_ratio, "reason": f"15S {signal} body={fmt(body_ratio,2)}"}
...
def analyze(symbol, oi_change=None):
    ...
    confirm15 = get_15s_confirmation(symbol)

    # Indicators are confirmation/context. 5M PA remains the main gate; the
    # completed 15S candle is the final trigger and must agree with direction.
    # This avoids entering merely because a 5M setup exists.
    #
    ...
    if signal == "BUY" and confirm15.get("signal") != "BUY":
        signal = "NONE"
    if signal == "SELL" and confirm15.get("signal") != "SELL":
        signal = "NONE"
    if RETEST_REQUIRED and signal != "NONE" and pa.get("retest") not in ("BULLISH", "BEARISH"):
        signal = "NONE"
    ...
    return {
        ...
        "fee_buffer_ok": fee_ok, "fee_buffer_usdt": FEE_BUFFER_USDT,
        "confirm15": confirm15, "entry_confirm_15s": confirm15.get("signal"),
        "reason": reason + f" | 15S={confirm15.get('signal')}" ,
    }
...
def startup_checks():
    ...
    log(f"SCORE priority={max(MIN_SCORE, MAJOR_MIN_SCORE)} | non-priority={NON_PRIORITY_MIN_SCORE} | max=9 (4 indicator + 5 PA stages) | ENGINE={VERSION} | SR+SWEEP+RECLAIM+BOS+RETEST=PA_COMPLETE")
