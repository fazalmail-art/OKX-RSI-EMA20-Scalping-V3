...
# =========================================================
# OKX SCALPING BOT V13.1 SIX-POINT CONFLUENCE (FIXED)
...
#   4. Explicit PA_COMPLETE remains the price-action safety gate. Retest is
#      treated as a separate quality/confluence point in the six-point score,
#      while the core PA gate requires SR + sweep + reclaim + fresh BOS.
...
# Six independent evidence buckets are scored. A trade requires 5/6.
# Session is a hard execution filter, never a score point. OI is a safety
# filter, never a score point.
...
def price_action_structure(candles):
    ...
    # CORE PA GATE:
    # Retest is downstream of BOS in this engine, so "BOS OR RETEST" is not
    # actually two independent alternatives: a retest cannot exist without
    # a BOS. The clean rule is therefore SR + SWEEP + RECLAIM + FRESH BOS.
    # Retest remains a separate sixth-score quality bucket and is preferred,
    # but is not double-counted or used as a fake alternative to BOS.
...
def analyze(symbol, oi_change=None):
    ...
    # =========================================================
    # V13.0 — SIX-POINT CONFLUENCE SCORE
    # =========================================================
    # The score deliberately uses independent evidence buckets rather than
    # counting every indicator as a separate vote. This avoids double-counting
    # several transformations of the same price series.
    #
    # 1) HTF trend alignment (15m)
    # 2) Liquidity location/reaction: SR + sweep + reclaim
    # 3) Structure shift: BOS/MSS
    # 4) Retest/acceptance of broken structure or OB
    # 5) Market participation: volume + ATR quality
    # 6) Momentum alignment: EMA20 + VWAP + MACD (2 of 3)
    #
    # Session is deliberately NOT a score point. It remains a hard execution
    # filter: an inactive session can block a trade, but it can never add score.
    # OI remains a safety filter, not a score point.
    ...
    # Participation/flow is one bucket: at least 2 of 3 must agree.
    # This prevents ADX, volume and ATR from becoming three correlated votes.
    flow_ok = sum([
        adx_v >= ADX_MIN,
    ...
    filters = {
        # ADX/volume/ATR are represented by the single FLOW bucket above.
        # They are deliberately not duplicated as separate hard vetoes.
        "SESSION": info["active"],
        "OI": True if oi_change is None else oi_change >= -OI_UNWIND_PCT,
    }
    # V13.1: the six-point engine is hard-capped at 5/6.
    # Railway may still contain legacy MIN_SCORE=7/8 values; those must not
    # make the required score impossible (or produce misleading need=7/8).
    ...
    if configured_required != required:
        log(f"SCORE_CONFIG_CLAMP configured={configured_required} -> required={required} (V13.1 hard cap 5/6)")
...
def startup_checks():
    ...
    log(f"SCORE required={min(max(MIN_SCORE, MAJOR_MIN_SCORE, NON_PRIORITY_MIN_SCORE), 5)} | max=6 | ENGINE={VERSION} | SCORE=HTF+LIQUIDITY+BOS+RETEST+FLOW+MOMENTUM | PA=SR+SWEEP+RECLAIM+FRESH_BOS | SESSION=FILTER_ONLY | SR_ATR={SR_ATR_MULT} | SR_PCT={SR_PCT}% | BOS_MAX_AGE={PA_BOS_MAX_AGE_BARS}")
