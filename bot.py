...
# Minimum improvement required before replacing OCO.
# Prevents constant cancel/recreate cycles.
...
PROTECTION_CANCEL_WAIT_SECONDS = int(
    os.getenv(
        "PROTECTION_CANCEL_WAIT_SECONDS",
        "8"
    )
)

PROTECTION_VERIFY_SECONDS = int(
    os.getenv(
        "PROTECTION_VERIFY_SECONDS",
        "8"
    )
)

PROTECTION_RETRIES = int(
    os.getenv(
        "PROTECTION_RETRIES",
        "4"
    )
...
def cancel_existing_protection(
    ...
    try:
        ...
        raise RuntimeError(
            "OLD OCO STILL PENDING AFTER "
            f"{PROTECTION_CANCEL_WAIT_SECONDS}s"
        )

    except Exception as error:

        log(
            "PROTECTION CANCEL ERROR | "
            f"{symbol} | "
            f"{error}"
        )
...
def place_full_position_protection(
    ...
    # IMPORTANT:
    #
    # For full-position TPSL on SWAP/NET:
    #
    # closeFraction = "1"
    #
    # MUST NOT be combined with "sz".
    #
    # OKX also requires reduceOnly=true
    # when posSide=net.
    #
    payload = {
...
# =========================================================
# CREATE / RECREATE PROTECTION SAFELY
# =========================================================

def replace_protection_safely(
    symbol,
    side,
    new_sl,
    tp_price,
    ...
    for attempt in range(
        ...
        try:
            ...
            # -------------------------------------------------
            # 1. Confirm position still exists
            # -------------------------------------------------
            ...
            if not position:
                ...
                raise RuntimeError(
                    ...
                    "before protection update"
                )

            # -------------------------------------------------
            # 2. Cancel old protection
            # -------------------------------------------------

            cancel_existing_protection(
                symbol
            )

            # -------------------------------------------------
            # 3. Confirm old protection is gone
            # -------------------------------------------------
            ...
            if protection_exists(
                ...
                raise RuntimeError(
                    "Old protection still "
                    "visible after cancellation"
                )

            # -------------------------------------------------
            # 4. Place new full-position OCO
            # -------------------------------------------------
            ...
            if not verified:
                ...
                raise RuntimeError(
                    "New protection submitted "
                    "but verification failed"
                )

            log(
                "PROTECTION REPLACED + VERIFIED | "
                f"{symbol} | "
                f"SL={fmt(new_sl)} | "
                f"TP={fmt(tp_price)} | "
                f"attempt={attempt}"
            ...
            log(
                "PROTECTION REPLACEMENT RETRY | "
                f"{symbol} | "
                f"attempt={attempt}/"
                f"{PROTECTION_RETRIES} | "
                f"{error}"
            )
...
# =========================================================
# EMERGENCY PROTECTION RECOVERY
# =========================================================

def emergency_protection_recovery(
    symbol,
    side,
    sl,
    tp,
    ...
    try:
        ...
        # If protection is already there,
...
def place_order(
    ...
    with order_lock:
        ...
        try:
            ...
            log(
                "OLD PROTECTION CLEANUP WARNING | "
                f"{symbol} | {error}"
            )
        ...
        if not verified:
            ...
            try:
                ...
                raise RuntimeError(
                    "CRITICAL: PROTECTION NOT "
                    "VERIFIED AND EMERGENCY CLOSE "
                    f"FAILED | {error}"
                )
...
def manage_position_protection(
    ...
    # -----------------------------------------------------
    # VERIFY EXISTING PROTECTION FIRST
    # -----------------------------------------------------
    ...
    if not protection_active:
        ...
        try:
            ...
            log(
                "CRITICAL PROTECTION RESTORE "
                f"FAILED | {symbol} | {error}"
            )

            # Do not silently leave an unprotected
            # position.
    ...
    try:
        ...
        log(
            "CRITICAL DYNAMIC PROTECTION ERROR | "
            f"{symbol} | "
            f"{error}"
        )
        ...
        # IMPORTANT:
        #
        # The old OCO was intentionally canceled.
        # Therefore we MUST NOT simply continue.
        #
        # Try to restore old protection.
        # -------------------------------------------------
...
def startup_checks():
    ...
    log(
        "DEMO + AUTO TRADE + PROTECTED POSITION"
    )

    log(
        "FULL POSITION OCO + BREAK EVEN + TRAILING SL"
    )
    ...
    log(
        f"BE_TRIGGER={BREAK_EVEN_TRIGGER_PCT}%"
    )

    log(
        f"BE_OFFSET={BREAK_EVEN_OFFSET_PCT}%"
    )

    log(
        f"TRAIL_START={TRAIL_START_PCT}%"
    )

    log(
        f"TRAIL_DISTANCE={TRAIL_DISTANCE_PCT}%"
    )

    log(
        f"MIN_SL_MOVE={MIN_SL_MOVE_PCT}%"
    )
