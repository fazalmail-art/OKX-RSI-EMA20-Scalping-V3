...
def analyze(symbol, oi_change=None):
    ...
    # FIX: the top-level sr/sweep/sweep_reclaim/bos/retest fields always show
    # whichever side (bull or bear) happened to be non-None, defaulting to
    # bullish -- so a bearish-only sequence under a RESISTANCE level could
    # print "SWEEP=BULLISH" even though the actual bearish stage list is
    # what's being evaluated. That's misleading for audit even though the
    # PA_COMPLETE_BUY/SELL decision itself is computed from the correct,
    # separate per-direction stage lists. Report both directions explicitly.
    reason = (
