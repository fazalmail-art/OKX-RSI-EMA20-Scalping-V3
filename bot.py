...
def send_alert(text, symbol=None):
    """Optional Telegram alert; failures never stop trading loop."""
    if not (TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID):
        return
    key = symbol or "global"
    if time.time() - last_alert.get(key, 0) < ALERT_COOLDOWN_SECONDS:
        return
    try:
        r = session.post(f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage", json={"chat_id": TELEGRAM_CHAT_ID, "text": text}, timeout=10)
        r.raise_for_status()
        last_alert[key] = time.time()
    except Exception as exc:
        print(f"[{iso_now()}] ALERT WARNING {exc}", flush=True)
...
def detect_signal(symbol):
    ...
    sl_pct_preview = max(MIN_SL_PCT, min(MAX_SL_PCT, av / close * 100 * SL_ATR_MULT))
    tp_pct_preview = av / close * 100 * TP_ATR_MULT
    projected_sl_buy = close * (1 - sl_pct_preview / 100)
    projected_tp_buy = close * (1 + tp_pct_preview / 100)
    projected_sl_sell = close * (1 + sl_pct_preview / 100)
    projected_tp_sell = close * (1 - tp_pct_preview / 100)
...
def open_trade(symbol, analysis, snap):
    ...
    with order_lock:
        ...
        send_alert(f"TRADE OPENED {symbol}\nSide: {side.upper()}\nEntry: {fmt(entry)}\nSL: {fmt(protection['sl'])}\nTP: {fmt(protection['tp'])}\nNotional: ${fmt(notional, 2)}", symbol + "_opened")
        ...
        try:
            ...
            for symbol in SYMBOLS:
                try:
                    ...
                    if a.get("signal") == "BUY":
                        send_alert(f"BUY SIGNAL {symbol}\nEntry: {fmt(a['entry'])}\nSL: {fmt(a['projected_sl_buy'])}\nTP: {fmt(a['projected_tp_buy'])}\nScore: {a['score']}/8\n{a['reason']}", symbol)
                    elif a.get("signal") == "SELL":
                        send_alert(f"SELL SIGNAL {symbol}\nEntry: {fmt(a['entry'])}\nSL: {fmt(a['projected_sl_sell'])}\nTP: {fmt(a['projected_tp_sell'])}\nScore: {a['score']}/8\n{a['reason']}", symbol)
