...
def price_action_structure(candles):
    ...
    # PA score uses five explicit stages:
    # SR -> Sweep -> Reclaim -> BOS -> Retest.
    # The entry gate is intentionally relaxed to 4/5.
    # SR + Sweep + Reclaim remain mandatory, and BOS or Retest completes
    # the directional PA gate. Candle rejection and OB remain separate
    # confirmations and do not distort the PA score.
    ...
    bull_stage = [
        sr == "SUPPORT",
        bull_sweep_idx is not None,
        bull_reclaim_idx is not None,
        bull_bos_idx is not None and bull_bos_idx > bull_reclaim_idx,
        bull_retest_recent and bull_retest_idx is not None and bull_bos_idx is not None and bull_retest_idx > bull_bos_idx,
    ]
    bear_stage = [
        sr == "RESISTANCE",
        bear_sweep_idx is not None,
        bear_reclaim_idx is not None,
        bear_bos_idx is not None and bear_bos_idx > bear_reclaim_idx,
        bear_retest_recent and bear_retest_idx is not None and bear_bos_idx is not None and bear_retest_idx > bear_bos_idx,
    ]
    ...
    # Relaxed 4/5 PA gate. The first three stages are mandatory;
    # BOS or Retest is required as the final confirmation.
    pa_complete_buy = (
        bull_stage[0]
        and bull_stage[1]
        and bull_stage[2]
        and (bull_stage[3] or bull_stage[4])
        and pa_buy >= PA_MIN_COMPLETE
    )
    ...
    pa_complete_sell = (
        ...
        and bear_stage[1]
        and bear_stage[2]
        and (bear_stage[3] or bear_stage[4])
        and pa_sell >= PA_MIN_COMPLETE
    )
    ...
    return {
        ...
        "pa_min_complete": PA_MIN_COMPLETE,
        "pa_buy": pa_buy,
        "pa_sell": pa_sell,
...
def analyze(symbol, oi_change=None):
    ...
    # Max score is 9: 4 indicator votes + 5 PA stages. PA is NOT double-counted.
    ...
    reason = (
        ...
        f"RETEST={pa['retest']} | PA={pa['pa_score']}/5 | BASE={max(base_buy,base_sell)}/4 | "
    ...
    if direction == "none":
        reason += " | PA_VETO=NO_4_OF_5_SR_SWEEP_RECLAIM_BOS_RETEST"
...
def startup_checks():
    ...
    log(f"SCORE priority={max(MIN_SCORE, MAJOR_MIN_SCORE)} | non-priority={NON_PRIORITY_MIN_SCORE} | max=8 | ENGINE=V12.4-PA-RELAXED-4OF5 | SR+SWEEP+RECLAIM+BOS/RETEST+CANDLE+OB")
