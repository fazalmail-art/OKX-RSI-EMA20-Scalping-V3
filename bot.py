...
def analyze(symbol, oi_change=None):
    ...
    if len(cs) < 40 or len(trend_cs) < 22:
        return {"signal": "NONE", "score": 0, "max_score": 8, "required_score": 99, "session": info["name"], "priority_session": info["priority"], "reason": "Not enough candles"}
    ...
    if any(x is None for x in (e20[i], r14[i], atr_v, macd_line[i], macd_sig[i], vw)):
        return {"signal": "NONE", "score": 0, "max_score": 8, "required_score": 99, "session": info["name"], "priority_session": info["priority"], "reason": "Indicator unavailable"}
    ...
    score = max(buy, sell)
    required = (MIN_SCORE if symbol in MEME_SYMBOLS else MAJOR_MIN_SCORE) if info["priority"] else NON_PRIORITY_MIN_SCORE
    signal = "NONE"
    ...
    return {
        ...
        "fee_buffer_usdt": FEE_BUFFER_USDT, "reason": " | ".join(reasons),
    }
...
def startup_checks():
    ...
    log(f"SCORE priority=6 | non-priority={NON_PRIORITY_MIN_SCORE} | max=8")
