...
PARTIAL_TP_ENABLE = os.getenv("PARTIAL_TP_ENABLE", "true").lower() == "true"
...
try:
    CORRELATION_GROUPS = json.loads(os.getenv(
        "CORRELATION_GROUPS",
        '{"L1_ALTS": ["ETH-USDT-SWAP", "SOL-USDT-SWAP", "NEAR-USDT-SWAP"], "MEME": ["DOGE-USDT-SWAP"]}'
    ))
except Exception:
    CORRELATION_GROUPS = {}
...
def calculate_initial_sl_tp(side, entry, tick, atr_pct=None):
...
def open_partial_protection(symbol, side, entry, full_size, info, tick, atr_pct=None):
    """
    ...
    SL_PERCENT/TP_PERCENT constants.
    ...
    """
    sl, tp = calculate_initial_sl_tp(side, entry, tick, atr_pct)
...
def place_order(symbol, analysis, snapshot):
    ...
    with order_lock:
        ...
        try:
            protection_state = open_partial_protection(symbol, side, entry, full_size, info, info["tickSz"], atr_pct_for_sizing)
        except Exception as exc:
